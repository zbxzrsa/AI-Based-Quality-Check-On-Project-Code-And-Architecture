...
class FallbackCache:
    ...
    def set(self, key: str, value: Any, ttl: float | None = None) -> None:
        ...
        self._cache[key] = {"value": value, "timestamp": time.time(), "ttl": ttl or self.default_ttl}
...
class DatabaseFallbackManager:
    ...
    def register_service(self, name: str, health_check: Callable | None = None) -> None:
        ...
        for attempt in range(self.config.max_retries + 1):
            try:
                ...
                if attempt < self.config.max_retries:
                    ...
                    delay = min(
                        self.config.retry_delay * (self.config.backoff_multiplier**attempt), self.config.max_retry_delay
                    )
                    logger.warning(
                        f"Service {service_name} attempt {attempt + 1} failed: {e}. Retrying in {delay:.1f}s"
                    )
    ...
    async def _execute_fallback(
        self, service_name: str, fallback_operation: Callable[[], T] | None, cache_key: str | None
    ) -> T:
        ...
        raise RuntimeError(f"Service {service_name} unavailable and no fallback configured")
    ...
    def get_system_status(self) -> dict[str, Any]:
        ...
        healthy_services = sum(1 for health in self.services.values() if health.state == ServiceState.HEALTHY)
        degraded_services = sum(1 for health in self.services.values() if health.state == ServiceState.DEGRADED)
        unavailable_services = sum(1 for health in self.services.values() if health.state == ServiceState.UNAVAILABLE)
...
async def execute_with_fallback(
    ...
    return await fallback_manager.execute_with_fallback(service_name, primary_operation, fallback_operation, cache_key)


def register_service_fallback(service_name: str, health_check: Callable | None = None) -> None:
