    ...
    async def _validate_pool_health(self, service: str, stats: PoolStats) -> bool:
        ...
        if service == "PostgreSQL" and self.connection_manager._postgresql_pool:
            ...
            try:
                ...
                await asyncio.wait_for(self.connection_manager._neo4j_driver.verify_connectivity(), timeout=5.0)
    ...
    async def _verify_pool_recovery(self, service: str) -> None:
        ...
        try:
            if service == "PostgreSQL" and self.connection_manager._postgresql_pool:
                ...
                # Test Neo4j driver
                await asyncio.wait_for(self.connection_manager._neo4j_driver.verify_connectivity(), timeout=10.0)
