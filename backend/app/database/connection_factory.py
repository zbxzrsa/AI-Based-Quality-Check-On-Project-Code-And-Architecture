        ...
        async with self._lock:
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    logger.warning(f"❌ {self.db_type.value} connection attempt {attempt}/{max_retries} failed: {e}")
...
class DatabaseConnectionManager:
    ...
    def get(self, db_type: DatabaseType) -> DatabaseConnection | None:
        """
    ...
    async def initialize_all(self, parallel: bool = True):
        ...
        """
        if parallel:
            tasks = [conn.initialize() for conn in self._connections.values()]
            results = await asyncio.gather(*tasks, return_exceptions=True)

            # Log any failures
            for conn, result in zip(self._connections.values(), results, strict=False):
                if isinstance(result, Exception):
                    logger.error(f"Failed to initialize {conn.db_type.value}: {result}")
        else:
            for conn in self._connections.values():
                try:
                    await conn.initialize()
                except Exception as e:
                    logger.error(f"Failed to initialize {conn.db_type.value}: {e}")
    ...
    def get_status_all(self) -> dict[str, Any]:
        ...
        Returns:
            Status dictionary
        """
        return {db_type.value: conn.get_status() for db_type, conn in self._connections.items()}
