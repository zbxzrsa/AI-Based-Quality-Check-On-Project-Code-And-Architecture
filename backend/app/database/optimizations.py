...
class DatabaseOptimizer:
    ...
    async def apply_performance_indexes(self, db: AsyncSession) -> dict[str, Any]:
        ...
        try:
            ...
            return {"created_indexes": created_indexes, "total_created": len(created_indexes), "status": "success"}
        ...
        try:
            ...
            for project in result:
                project_dict = {
                    ...
                    "avg_review_score": sum(r.score for r in project.reviews) / len(project.reviews)
                    if project.reviews
                    else 0,
    ...
    async def run_maintenance_tasks(self, db: AsyncSession) -> dict[str, Any]:
        ...
        try:
            ...
            return {"status": "completed", "maintenance_results": maintenance_results, "timestamp": time.time()}
    ...
    async def analyze_query_performance(self, db: AsyncSession) -> dict[str, Any]:
        ...
        try:
            return {"slow_queries": [], "table_stats": [], "unused_indexes": []}
        except Exception as e:
    ...
    async def get_projects_optimized(
        ...
        query = (
            db.query(Project)
            .options(selectinload(Project.pull_requests), selectinload(Project.owner))
            .filter(Project.owner_id == user_id)
        )
        ...
        return [
            {
                ...
                "description": project.description,
                "status": project.status,
                "owner": {"id": project.owner.id, "username": project.owner.username, "email": project.owner.email}
                if project.owner
                else None,
                "created_at": project.created_at.isoformat(),
                "updated_at": project.updated_at.isoformat(),
                ...
                "pull_requests": [
                    {"id": str(pr.id), "title": pr.title, "status": pr.status, "created_at": pr.created_at.isoformat()}
        ...
        try:
            ...
            if stats.idle_in_transaction > 5:
                recommendations.append(
                    "High number of idle in transaction connections - check for long-running transactions"
                )
