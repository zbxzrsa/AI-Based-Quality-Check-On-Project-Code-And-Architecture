...
from app.database.models import CompatibilityResult, DatabaseConfig, get_python_version
    ...
    def validate_environment_variables(self) -> list[str]:
        ...
        for var_name, description in self.REQUIRED_ENV_VARS.items():
            ...
            if not value or not value.strip():
                errors.append(
                    f"Required environment variable '{var_name}' is missing or empty. Description: {description}"
                )
    ...
    def validate_configuration_conflicts(self) -> list[str]:
        ...
        if environment == "production":
            ...
            if len(jwt_secret) < 32:
                conflicts.append("Security conflict: JWT_SECRET must be at least 32 characters in production")

            # Production database requirements
            if os.environ.get("POSTGRES_PASSWORD") == "password":
                conflicts.append("Security conflict: Default password 'password' not allowed in production")

            if os.environ.get("NEO4J_PASSWORD") == "password":
                conflicts.append("Security conflict: Default Neo4j password 'password' not allowed in production")
