...
class ConnectionManager:
    ...
    def __init__(self, config: DatabaseConfig | None = None):
        ...
        self.postgresql_client = PostgreSQLClient(connection_manager=self, retry_manager=self.retry_manager)
        ...
        self.pool_stats = {
            "PostgreSQL": PoolStats(
                service="PostgreSQL", max_size=self.pool_config.max_size, min_size=self.pool_config.min_size
            ),
            "Neo4j": PoolStats(service="Neo4j", max_size=self.pool_config.max_size, min_size=self.pool_config.min_size),
        }
    ...
    async def _create_postgresql_pool(self) -> asyncpg.Pool:
        ...
        pool_config = {
            ...
            "server_settings": {"application_name": "ai_code_review_platform", "timezone": "UTC"},
    ...
    def _start_background_tasks(self) -> None:
        ...
        self._pool_cleanup_task = asyncio.create_task(self._pool_cleanup_monitor(), name="pool_cleanup_monitor")
    ...
    async def _validate_pool_health(self, service: str, stats: PoolStats) -> bool:
        ...
        if service == "PostgreSQL" and self._postgresql_pool:
            ...
            try:
                ...
                await asyncio.wait_for(self._neo4j_driver.verify_connectivity(), timeout=5.0)
    ...
    async def _verify_pool_recovery(self, service: str) -> None:
        ...
        try:
            if service == "PostgreSQL" and self._postgresql_pool:
                ...
                # Test Neo4j driver
                await asyncio.wait_for(self._neo4j_driver.verify_connectivity(), timeout=10.0)
        ...
        while recreation_attempt < max_recreation_attempts:
            try:
                ...
                if await self._test_connection_health(connection):
                    ...
                    logger.warning(
                        f"Unhealthy PostgreSQL connection detected, attempting recreation (attempt {recreation_attempt + 1})"
                    )
                    if connection:
                        try:
    ...
    async def verify_postgres(self) -> ConnectionStatus:
        ...
        try:
            ...
            return ConnectionStatus(
                service="PostgreSQL", is_connected=False, error=error_msg, is_critical=True, last_attempt=datetime.now()
            )
    ...
    async def verify_neo4j(self) -> ConnectionStatus:
        ...
        try:
            ...
            return ConnectionStatus(
                service="Neo4j", is_connected=False, error=error_msg, is_critical=False, last_attempt=datetime.now()
            )
    ...
    async def verify_all(self) -> dict[str, ConnectionStatus]:
        ...
        postgres_status, neo4j_status, redis_status = await asyncio.gather(
            self.verify_postgres(), self.verify_neo4j(), self.verify_redis(), return_exceptions=False
        )
    ...
    async def get_pool_statistics(self) -> dict[str, dict[str, Any]]:
        ...
        for service in ["PostgreSQL", "Neo4j"]:
            operation_type = (
                OperationType.POSTGRESQL_CONNECTION if service == "PostgreSQL" else OperationType.NEO4J_CONNECTION
            )
    ...
    async def _get_enhanced_pool_metrics(self, service: str, stats: PoolStats) -> dict[str, Any]:
        ...
        if service == "PostgreSQL" and self._postgresql_pool:
            try:
                ...
                if stats.max_size > 0:
                    metrics["pool_utilization"] = round((metrics["actual_active_size"] / stats.max_size) * 100, 1)
    ...
    async def _get_system_health_metrics(self) -> dict[str, Any]:
        ...
        metrics = {
            ...
            "background_tasks_running": (self._health_check_task is not None and not self._health_check_task.done())
            and (self._pool_cleanup_task is not None and not self._pool_cleanup_task.done()),
    ...
    async def get_detailed_pool_report(self) -> dict[str, Any]:
        ...
        Validates Requirements: 4.5
        """
        report = {"timestamp": datetime.now().isoformat(), "summary": {}, "services": {}, "recommendations": []}
        ...
        for service, service_stats in stats.items():
            ...
            if service_stats.get("connection_timeouts", 0) > 5:
                ...
                report["recommendations"].append(
                    f"Consider increasing {service} connection timeout or checking network"
                )
