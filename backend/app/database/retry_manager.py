...
class RetryManager:
    ...
    def should_retry(self, exception: Exception, attempt: int, operation_type: OperationType) -> bool:
        ...
        if any(keyword in exception_name.lower() for keyword in ["connection", "network", "socket", "dns"]):
            return True
    ...
    def _log_retry_exhaustion(
        ...
        logger.error(
            ...
            extra={
                ...
                "first_failure_time": retry_state.first_failure_time.isoformat()
                if retry_state.first_failure_time
                else None,
                "last_failure_time": retry_state.last_failure_time.isoformat()
                if retry_state.last_failure_time
                else None,
    ...
    def _get_resolution_steps(self, operation_type: OperationType, exception: Exception | None) -> list[str]:
        """Generate resolution guidance based on operation type and error"""
        steps = []
    ...
    def get_retry_statistics(self, operation_type: OperationType | None = None) -> dict[str, Any]:
        """
        ...
        """
        stats = {"total_operations": len(self.retry_states), "operations_by_type": {}, "failure_patterns": {}}

        for _state_key, state in self.retry_states.items():
