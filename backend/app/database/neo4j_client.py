...
from .models import DatabaseConfig, DatabaseError, ErrorType
from .retry_manager import OperationType, RetryManager
...
class AuthFailureTracker:
    ...
    def record_auth_failure(self, error: AuthError) -> None:
        ...
        if any(keyword in error_msg for keyword in ["rate limit", "too many", "throttle", "blocked"]):
    ...
    def record_auth_success(self) -> None:
        ...
        if self.failure_count > 0:
            logger.info(
                ...
                extra={"total_failures": self.failure_count, "recovery_time": self.last_success_time.isoformat()},
            )
...
class SessionManager:
    ...
    def __init__(self, client: "Neo4jClient"):
        ...
        self.session_stats = {"created": 0, "closed": 0, "failed": 0, "active_count": 0}
        self._cleanup_lock = asyncio.Lock()

    @asynccontextmanager
    async def get_session(self, database: str | None = None):
        """
    ...
    async def _create_session(self, database: str | None = None) -> AsyncSession:
        """
        ...
        logger.debug("Neo4j session created", extra={"database": database or "default"})
    ...
    async def _close_session_safely(self, session: AsyncSession) -> None:
        ...
        try:
            ...
            logger.warning("Error closing Neo4j session", extra={"error": str(e), "error_type": type(e).__name__})
    ...
    async def close_all_sessions(self) -> None:
        ...
        async with self._cleanup_lock:
            ...
            close_tasks = [self._close_session_safely(session) for session in sessions_to_close]
    ...
    def get_session_statistics(self) -> dict[str, Any]:
        ...
        """
        return {**self.session_stats, "active_count": len(self.active_sessions)}
...
class Neo4jClient:
    ...
    def _configure_retry_policies(self) -> None:
        ...
        self.retry_manager.configure_operation(OperationType.NEO4J_AUTHENTICATION, auth_config)

        # Connection retry policy
        connection_config = self.config.retry_config
        self.retry_manager.configure_operation(OperationType.NEO4J_CONNECTION, connection_config)
        ...
        async with self._connection_lock:
            if self._driver is not None:
                try:
                    ...
                    logger.warning(
                        "Existing Neo4j driver failed connectivity check, recreating", extra={"error": str(e)}
                    )
                    await self._close_driver()

            # Create new driver with retry logic
            self._driver = await self.retry_manager.execute_with_retry(
                self._create_driver, OperationType.NEO4J_AUTHENTICATION, operation_id="neo4j_driver_creation"
            )
    ...
    async def _create_driver(self) -> AsyncDriver:
        ...
        """
        try:
            logger.debug(f"Creating Neo4j driver for {self.config.neo4j_uri}", extra={"uri": self.config.neo4j_uri})
            ...
            logger.error("Neo4j service unavailable", extra={"uri": self.config.neo4j_uri, "error": str(e)})
            raise

        except Exception as e:
            logger.error(
                "Unexpected error creating Neo4j driver",
                extra={"uri": self.config.neo4j_uri, "error": str(e), "error_type": type(e).__name__},
                exc_info=True,
            )
            raise

    async def get_session(self, database: str | None = None) -> AsyncSession:
        """
        ...
        try:
            session = driver.session(database=database)
            logger.debug("Neo4j session created (deprecated method)", extra={"database": database or "default"})
            return session

        except Exception as e:
            logger.error(
                "Failed to create Neo4j session",
                extra={"database": database or "default", "error": str(e), "error_type": type(e).__name__},
            )
            raise
    ...
    async def _execute_query_impl(self, query: str, parameters: dict[str, Any], database: str | None) -> neo4j.Result:
        """
        ...
        async with self.session_manager.get_session(database) as session:
            try:
                ...
                logger.debug("Neo4j query executed successfully", extra={"database": database or "default"})
                ...
                logger.error(
                    ...
                    extra={"consecutive_failures": self.auth_tracker.consecutive_failures, "error": str(e)},
                )
                raise
    ...
    async def execute_transaction(self, transaction_function, database: str | None = None, **kwargs) -> Any:
        """
    ...
    async def _execute_transaction_impl(self, transaction_function, database: str | None, **kwargs) -> Any:
        """
        ...
        async with self.session_manager.get_session(database) as session:
            try:
                logger.debug(
                    ...
                    extra={"function": transaction_function.__name__, "database": database or "default"},
                )
    ...
    async def test_connectivity(self) -> bool:
        ...
        try:
            ...
            logger.error("Neo4j connectivity test failed", extra={"error": str(e), "error_type": type(e).__name__})
            return False
    ...
    async def get_auth_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "first_failure_time": (
                self.auth_tracker.first_failure_time.isoformat() if self.auth_tracker.first_failure_time else None
            ),
            "last_failure_time": (
                self.auth_tracker.last_failure_time.isoformat() if self.auth_tracker.last_failure_time else None
            ),
            "last_success_time": (
                self.auth_tracker.last_success_time.isoformat() if self.auth_tracker.last_success_time else None
            ),
        }
    ...
    async def _close_driver(self) -> None:
        ...
        if self._driver is not None:
            try:
                ...
                logger.warning("Error closing Neo4j driver", extra={"error": str(e)})
            finally:
                self._driver = None
    ...
    async def execute_read_query(
        ...
        async with self.session_manager.get_session(database) as session:
            try:
                ...
                result = await session.execute_read(lambda tx: tx.run(query, parameters or {}))

                logger.debug("Neo4j read query executed successfully", extra={"database": database or "default"})
    ...
    async def execute_write_query(
        ...
        async with self.session_manager.get_session(database) as session:
            try:
                ...
                logger.debug("Neo4j write query executed successfully", extra={"database": database or "default"})
    ...
    async def get_client_statistics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "retries": retry_stats,
            "driver_status": {"is_connected": self._driver is not None, "uri": self.config.neo4j_uri},
        }
