...
from app.database.models import CompatibilityResult, ConnectionStatus, DatabaseError, ErrorType
from app.database.postgresql import AsyncSessionLocal, close_postgres, init_postgres, test_postgres_connection
...
class PostgreSQLClient:
    ...
    def __init__(self, connection_manager=None, retry_manager: RetryManager | None = None):
        """
        ...
        self._compatibility_result: CompatibilityResult | None = None
        ...
        try:
            ...
            if success:
                return ConnectionStatus(
                    service="PostgreSQL", is_connected=True, response_time_ms=response_time, last_attempt=datetime.now()
                )
            else:
                return ConnectionStatus(
                    service="PostgreSQL",
            ...
            await self._report_connection_error(
                error_type=ErrorType.CONNECTION_TIMEOUT, message=error_message, details={"error": str(e)}
            )
    ...
    def get_compatibility_result(self) -> CompatibilityResult | None:
        """
    ...
    async def _report_connection_error(self, error_type: ErrorType, message: str, details: dict[str, Any]) -> None:
        ...
        try:
            ...
            formatted_error = self.error_reporter.format_connection_error(
                service="PostgreSQL", error=Exception(message), connection_string=details.get("dsn", "N/A")
            )
