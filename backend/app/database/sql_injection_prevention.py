...
def validate_sql_identifier(identifier: str) -> str:
    ...
    if identifier.lower() in sql_keywords:
        raise SQLInjectionError(f"SQL keyword '{identifier}' cannot be used as an identifier")
...
def validate_order_by(order_by: str, allowed_columns: list[str]) -> str:
    ...
    if column not in allowed_columns:
        raise SQLInjectionError(
            f"Column '{column}' not allowed in ORDER BY. Allowed columns: {', '.join(allowed_columns)}"
        )

    # Validate direction
    if direction not in ("ASC", "DESC"):
        raise SQLInjectionError(f"Invalid sort direction: '{direction}'. Must be ASC or DESC")
...
def build_safe_where_clause(filters: dict[str, Any], allowed_columns: list[str]) -> tuple[str, dict[str, Any]]:
    ...
    for column, value in filters.items():
        ...
        if column not in allowed_columns:
            raise SQLInjectionError(
                f"Column '{column}' not allowed in WHERE clause. Allowed columns: {', '.join(allowed_columns)}"
            )
...
def audit_raw_sql(func):
    ...
    async def wrapper(*args, **kwargs):
        ...
        logger.warning(
            f"Raw SQL execution in {func.__module__}.{func.__name__}. Ensure parameterized queries are used."
        )
...
class SafeQueryBuilder:
    ...
    def build_select(
        ...
        if columns:
            validated_columns = [validate_sql_identifier(col) for col in columns if col in self.allowed_columns]
    ...
    def build_count(self, filters: dict[str, Any] | None = None) -> tuple[str, dict[str, Any]]:
        """
...
async def execute_safe_query(db: AsyncSession, query: str, parameters: dict[str, Any]) -> Any:
    """
