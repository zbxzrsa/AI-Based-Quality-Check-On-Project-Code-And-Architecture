...
class Neo4jWithCircuitBreaker:
    ...
    async def get_driver(self) -> AsyncDriver:
        ...
        try:
            ...
            raise Neo4jCircuitBreakerError("Neo4j database is temporarily unavailable. Please try again later.")

    async def execute_read_query(self, query: str, parameters: dict | None = None, database: str | None = None) -> list:
        """
        ...
        try:
            ...
            if cached_data is not None:
                logger.info("Returning cached Neo4j query result")
                return cached_data
            # No cached data available
            raise Neo4jCircuitBreakerError("Neo4j database is temporarily unavailable and no cached data is available.")

    async def execute_write_query(self, query: str, parameters: dict | None = None, database: str | None = None) -> Any:
        """
    ...
    async def execute_transaction(self, transaction_func: Callable, database: str | None = None) -> Any:
        """
...
# Singleton instance
_neo4j_with_cb: Neo4jWithCircuitBreaker | None = None
