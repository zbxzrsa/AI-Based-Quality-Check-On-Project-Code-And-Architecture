...
async def list_repositories(
    ...
    try:
        ...
        return RepositoryListResponse(repositories=list(repositories), total=total, page=page, page_size=page_size)
...
async def update_repository(
    ...
    try:
        ...
        if repository.created_by != current_user["sub"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to update this repository"
            )
...
async def delete_repository(
    ...
    try:
        ...
        if repository.created_by != current_user["sub"]:
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to delete this repository"
            )
...
async def sync_repository(
    ...
    try:
        ...
        if not repository:
            raise HTTPException(status_code=status.HTTP_404_NOT_IMPLEMENTED, detail="Repository not found")

        # Check ownership
        if repository.created_by != current_user["sub"]:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Not authorized to sync this repository")
        ...
        if not validation.is_valid:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Repository validation failed: {validation.error}"
            )
