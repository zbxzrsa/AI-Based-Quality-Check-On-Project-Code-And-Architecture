...
async def add_repository(
    ...
    try:
        ...
        logger.info(
            f"Repository added successfully: {repository.owner}/{repository.name} by user {current_user['sub']}"
        )
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to add repository")


@router.get("/validate", response_model=RepositoryValidationResult, summary="Validate a repository URL")
async def validate_repository(
    repository_url: str = Query(..., description="GitHub repository URL to validate"),
    branch: str | None = Query(None, description="Optional branch to validate"),
    db: Annotated[AsyncSession, Depends(get_db)] = None,
    current_user: Annotated[dict, Depends(get_current_user)] = None,
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to validate repository")
...
async def get_repository(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve repository")


@router.get("/", response_model=RepositoryListResponse, summary="List all repositories")
async def list_repositories(
    ...
    try:
        ...
        result = await service.list_entities(
            user_id=current_user["sub"], page=page, page_size=page_size, filters=filters
        )
        ...
        return RepositoryListResponse(
            repositories=result["items"], total=result["total"], page=result["page"], page_size=result["page_size"]
        )

    except Exception as e:
        logger.error(f"Error listing repositories: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to list repositories")


@router.patch("/{repository_id}", response_model=RepositoryResponse, summary="Update repository settings")
async def update_repository(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update repository")


@router.delete("/{repository_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Remove a repository")
async def delete_repository(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to delete repository")


@router.post("/{repository_id}/sync", response_model=RepositoryResponse, summary="Sync repository with remote")
async def sync_repository(
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to sync repository: {str(e)}"
        )


@router.post("/{repository_id}/archive", response_model=dict, summary="Archive a repository")
async def archive_repository(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to archive repository")


@router.post("/{repository_id}/reactivate", response_model=dict, summary="Reactivate a repository")
async def reactivate_repository(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to reactivate repository")


@router.get("/{repository_id}/status", response_model=dict, summary="Get repository status")
async def get_repository_status(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to get repository status")
