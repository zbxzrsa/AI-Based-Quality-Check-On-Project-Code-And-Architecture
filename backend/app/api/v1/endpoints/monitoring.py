...
from app.services.slow_query_monitor import SlowQueryMonitor, analyze_slow_queries, get_postgres_slow_queries
...
async def get_slow_queries(limit: int = 50, current_user: User = Depends(get_current_user)):
    """
    ...
    try:
        async for db in get_db():
            ...
            return {"success": True, "data": slow_queries, "count": len(slow_queries)}
    except Exception as e:
        logger.error(f"Failed to retrieve slow queries: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve slow queries")


@router.get("/slow-queries/stats")
async def get_slow_query_stats(current_user: User = Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to analyze slow queries")


@router.get("/slow-queries/monitor-stats")
async def get_monitor_stats(current_user: User = Depends(get_current_user)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve monitor statistics"
        )


@router.delete("/slow-queries/monitor-stats")
async def clear_monitor_entries(hours: int = 24, current_user: User = Depends(get_current_user)):
    """
    ...
    if current_user.role.value != "ADMIN":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only admins can clear monitor entries")

    try:
        slow_query_monitor.clear_old_entries(hours=hours)
        return {"success": True, "message": f"Cleared slow query monitor entries older than {hours} hours"}
    except Exception as e:
        logger.error(f"Failed to clear monitor entries: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to clear monitor entries")


@router.post("/slow-queries/configure-threshold")
async def configure_threshold(threshold_ms: float, current_user: User = Depends(get_current_user)):
    """
    ...
    """
    if current_user.role.value != "ADMIN":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only admins can configure threshold")
    ...
    try:
        ...
        return {"success": True, "message": f"Slow query threshold set to {threshold_ms}ms"}
    except Exception as e:
