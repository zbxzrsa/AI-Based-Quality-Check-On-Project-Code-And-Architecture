...
async def extract_pr_data(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not pr_data:
        raise ValidationException(message="Missing pull_request data in payload", field="pull_request")
...
async def queue_analysis_task(pr_id: str, project_id: str, pr_data: dict[str, Any]) -> dict[str, Any]:
    ...
    logger.info(
        ...
        extra={"task_id": task.id, "pr_id": pr_id, "project_id": project_id, "pr_number": pr_data.get("pr_number")},
    )

    return {"task_id": task.id, "status": "queued", "pr_id": pr_id, "pr_number": pr_data.get("pr_number")}
...
async def code_review_webhook(
    ...
    logger.info(
        ...
        extra={"event_type": x_github_event, "delivery_id": x_github_delivery, "action": payload.get("action")},
    )

    # Handle ping event
    if x_github_event == "ping":
        logger.info("Received ping event from GitHub")
        return {"message": "pong", "timestamp": start_time.isoformat()}

    # Only handle pull_request events
    if x_github_event != "pull_request":
        logger.warning(f"Unsupported event type: {x_github_event}")
        return {"message": f"Event type {x_github_event} not supported", "supported_events": ["pull_request"]}
    ...
    if x_github_delivery:
        ...
        if not is_new:
            ...
            return {"message": "Webhook already processed", "delivery_id": x_github_delivery}
    ...
    if not project:
        logger.warning(f"No project found for repository: {repo_url}", extra={"repository_url": repo_url})
        raise HTTPException(
    ...
    pr_stmt = select(PullRequest).where(PullRequest.project_id == project.id, PullRequest.github_pr_number == pr_number)
    ...
    if not pr:
        ...
        logger.info(
            ...
            extra={"pr_id": str(pr.id), "pr_number": pr_number, "project_id": str(project.id)},
        )
    else:
        # Update existing PR
        ...
        logger.info(
            f"Updated PR record: {pr_number}", extra={"pr_id": str(pr.id), "pr_number": pr_number, "action": action}
        )
    ...
    logger.info(
        ...
        extra={"pr_id": str(pr.id), "processing_time_seconds": processing_time, "task_id": task_info.get("task_id")},
    )
