...
from app.services.library_management.library_manager import ValidationError as LibraryValidationError
...
async def validate_library(
    ...
    try:
        ...
        validation_result = await library_manager.validate_library(
            uri=request.uri, context=request.project_context, user_id=str(current_user.id)
        )
        ...
        logger.error(
            ...
            extra={"user_id": str(current_user.id), "operation": "validate_library_endpoint", "error": str(e)},
        )
        return JSONResponse(status_code=status.HTTP_400_BAD_REQUEST, content={"valid": False, "errors": [str(e)]})
        ...
        return JSONResponse(
            ...
            content={"valid": False, "errors": ["Internal server error occurred during validation"]},
        )
    finally:
        # Clean up resources
...
async def install_library(
    ...
    try:
        ...
        if installation_result.success:
            logger.info(
                ...
                extra={
                    ...
                    "operation": "install_library_endpoint",
                    "success": True,
                    "library_name": installation_result.installed_library.name
                    if installation_result.installed_library
                    else None,
                    "library_version": installation_result.installed_library.version
                    if installation_result.installed_library
                    else None,
        ...
        logger.error(
            ...
            extra={"user_id": str(current_user.id), "operation": "install_library_endpoint", "error": str(e)},
        )
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"success": False, "errors": ["Internal server error occurred during installation"]},
        )
    finally:
        # Clean up resources
        await library_manager.close()
...
async def search_libraries(
    ...
    registry: str | None = Query(None, description="Registry type filter (npm, pypi, maven)"),
    current_user: User = Depends(get_current_user),
    library_manager: LibraryManager = Depends(get_library_manager),
):
    """
    ...
    try:
        ...
        response = SearchResponse(results=search_results, total=len(search_results))
        ...
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"results": [], "total": 0, "error": "Internal server error occurred during search"},
        )
    finally:
        # Clean up resources
        await library_manager.close()
...
async def list_installed_libraries(
    ...
    project_context: ProjectContext | None = Query(None, description="Filter by project context"),
    current_user: User = Depends(get_current_user),
    library_manager: LibraryManager = Depends(get_library_manager),
):
    """
    ...
    try:
        ...
        installed_libraries = await library_manager.get_installed_libraries(
            project_id=project_id, context=project_context, user_id=str(current_user.id)
        )

        # Convert to LibraryListResponse
        response = LibraryListResponse(libraries=installed_libraries, total=len(installed_libraries))
        ...
        if any(keyword in error_text for keyword in ["not found", "does not exist", "invalid project"]):
            return JSONResponse(
                ...
                content={"libraries": [], "total": 0, "error": f"Project not found: {project_id}"},
            )

        # Default to 500 for other errors
        return JSONResponse(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            content={"libraries": [], "total": 0, "error": "Internal server error occurred while retrieving libraries"},
        )
    finally:
        # Clean up resources
        await library_manager.close()
