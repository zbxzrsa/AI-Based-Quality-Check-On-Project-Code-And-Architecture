...
async def create_invitation(
    ...
    try:
        ...
        has_access = await ProjectInvitationService.has_project_access(db, current_user.id, project_id)
        if not has_access:
            raise HTTPException(
...
async def get_pending_invitations(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    """
    try:
        invitations = await ProjectInvitationService.get_pending_invitations_for_user(db, current_user.email)
...
async def accept_invitation(
    ...
    try:
        member = await ProjectInvitationService.accept_invitation(
            db=db, invitation_token=accept_data.invitation_token, user_id=current_user.id
        )
...
async def get_project_members(
    ...
    try:
        ...
        has_access = await ProjectInvitationService.has_project_access(db, current_user.id, project_uuid)
        if not has_access:
...
async def get_my_projects(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    """
    try:
        memberships = await ProjectInvitationService.get_user_project_memberships(db, current_user.id)
