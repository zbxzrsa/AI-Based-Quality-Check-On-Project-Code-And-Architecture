...
async def get_or_create_pr(project: Project, pr_metadata: dict[str, Any], db: AsyncSession) -> PullRequest:
    """
...
async def github_webhook_handler(
    ...
    if not repo_full_name:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Missing repository information in payload")
    ...
    if not project:
        ...
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Project not configured for repository: {repo_full_name}"
        )
    ...
    if event_type == "ping":
        ...
        return {"message": "pong", "project_id": str(project.id), "repository": repo_full_name}
        ...
        task_info = analyze_pull_request_sync(pr_id=str(pr.id), project_id=str(project.id))

        logger.info(f"Queued PR analysis: project={project.id}, pr={pr.github_pr_number}, task={task_info['task_id']}")
        ...
        return {"message": f"Event type '{event_type}' not supported", "event_type": event_type}
