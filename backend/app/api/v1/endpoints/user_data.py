...
from app.models import AuditLog, Project, PullRequest, User
...
async def export_user_data(
    user_id: str, request: Request, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)
):
    """
    ...
    if str(current_user.id) != user_id and current_user.role.value != "admin":
        logger.warning(
            f"Unauthorized data export attempt: user {current_user.id} tried to export data for user {user_id}"
        )
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You can only export your own data")
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to export user data")
...
async def delete_user_account(
    ...
    if str(current_user.id) != user_id and current_user.role.value != "admin":
        logger.warning(
            f"Unauthorized account deletion attempt: user {current_user.id} tried to delete account {user_id}"
        )
    ...
    if deletion_request.confirm_email != user.email:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Email confirmation does not match user email"
        )

    # Check if user is already scheduled for deletion
    if not user.is_active:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Account is already scheduled for deletion")
    ...
    try:
        ...
        audit_result = await db.execute(
            anonymize_audit_logs_query, {"user_id": uuid.UUID(user_id), "anonymized_user_id": anonymized_user_id}
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to process account deletion request"
        )
