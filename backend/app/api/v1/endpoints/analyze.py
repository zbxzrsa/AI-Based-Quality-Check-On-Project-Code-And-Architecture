...
class ArchitectureAnalyzer:
    ...
    async def _analyze_code_structure(self, repo_path: str) -> dict[str, Any]:
        ...
        code_hierarchy = {"files": [], "summary": {"total_files": 0, "file_types": {}}}
        ...
        for file_path in repo_dir.rglob("*"):
            if file_path.is_file():
                ...
                code_hierarchy["summary"]["file_types"][suffix] = (
                    code_hierarchy["summary"]["file_types"].get(suffix, 0) + 1
                )
                if suffix == ".py":
        ...
        async def process_file(file_path: Path) -> dict[str, Any] | None:
    ...
    async def _analyze_file(self, file_path: Path, repo_root: Path) -> dict[str, Any] | None:
    ...
    def _analyze_python_file(self, content: str, relative_path: str, file_type: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"path": relative_path, "type": file_type, "classes": classes, "functions": functions}

        except SyntaxError as e:
            logger.warning(f"Syntax error in {relative_path}: {str(e)}")
            return {"path": relative_path, "type": file_type, "classes": [], "functions": []}
            ...
            if isinstance(node, ast.ClassDef):
                ...
                for item in node.body:
                    if isinstance(item, ast.FunctionDef):
                        methods.append(
                            {"name": item.name, "line": item.lineno, "complexity": self._calculate_complexity(item)}
                        )
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef):
                if not self._is_function_inside_class(node, tree):
                    functions.append(
                        {"name": node.name, "line": node.lineno, "complexity": self._calculate_complexity(node)}
                    )
        return functions
    ...
    async def _analyze_architecture(self, code_hierarchy: dict[str, Any], metrics: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            architectural_purpose = await self._generate_architectural_purpose_ollama(architecture_data, ollama_client)

            # Detect architectural patterns
            detected_patterns = await self._detect_architectural_patterns_ollama(architecture_data, ollama_client)

            # Identify potential issues
            potential_issues = await self._identify_potential_issues_ollama(architecture_data, metrics, ollama_client)
    ...
    def _prepare_architecture_data(self, code_hierarchy: dict[str, Any], metrics: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "file_types": list({f["type"] for f in code_hierarchy["files"] if f["type"] != "unknown"}),
        }

    async def _generate_architectural_purpose_ollama(
        self, architecture_data: dict[str, Any], ollama_client: LLMClient
    ) -> str:
    ...
    async def _identify_potential_issues_ollama(
        self, architecture_data: dict[str, Any], metrics: dict[str, Any], ollama_client: LLMClient
    ) -> list[str]:
    ...
    def _store_in_neo4j(
        self, code_hierarchy: dict[str, Any], architecture_summary: dict[str, Any], metrics: dict[str, Any]
    ):
...
async def analyze_repository(request: AnalyzeRequest, background_tasks: BackgroundTasks):
    ...
    return StreamingResponse(analyze_and_stream(), media_type="text/event-stream")
