...
@router.get("/metrics")
async def get_performance_metrics(db: Annotated[AsyncSession, Depends(get_db)]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "api": {
                ...
                "avg_response_time": sum(m.get("duration", 0) for m in api_metrics) / len(api_metrics)
                if api_metrics
                else 0,
                "error_rate": len([m for m in api_metrics if m.get("status", 200) >= 400]) / len(api_metrics)
                if api_metrics
                else 0,
...
async def get_slow_queries(db: Annotated[AsyncSession, Depends(get_db)], limit: int = 20) -> dict[str, Any]:
...
async def get_index_recommendations(db: Annotated[AsyncSession, Depends(get_db)]) -> dict[str, Any]:
...
async def run_database_maintenance(
    background_tasks: BackgroundTasks, db: Annotated[AsyncSession, Depends(get_db)]
) -> dict[str, Any]:
...
async def warm_cache(background_tasks: BackgroundTasks) -> dict[str, Any]:
    """Warm cache with frequently accessed data"""
    try:
        from app.shared.cache_manager import warm_analytics_cache, warm_library_cache, warm_project_cache
...
async def get_performance_recommendations(db: Annotated[AsyncSession, Depends(get_db)]) -> dict[str, Any]:
