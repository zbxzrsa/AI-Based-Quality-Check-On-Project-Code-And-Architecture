...
class AuthMiddleware:
    ...
    def check_role(required_role: Role):
        ...
        async def role_checker(request: Request, credentials: HTTPAuthorizationCredentials) -> TokenPayload:
            ...
            if user_role != required_role:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN, detail=f"This action requires {required_role.value} role"
                )
    ...
    def check_permission(required_permission: Permission):
        ...
        async def permission_checker(
            request: Request, credentials: HTTPAuthorizationCredentials, db: AsyncSession = Depends(get_db)
        ) -> TokenPayload:
            # First authenticate the token
            payload = await AuthMiddleware.authenticate_token(request, credentials)
    ...
    def check_project_access(required_permission: Permission):
        ...
        async def project_access_checker(
            request: Request, credentials: HTTPAuthorizationCredentials, db: AsyncSession
        ) -> TokenPayload:
            # First authenticate the token
            payload = await AuthMiddleware.authenticate_token(request, credentials)
            ...
            can_access = await RBACService.can_access_project(db, payload.user_id, project_id, required_permission)
