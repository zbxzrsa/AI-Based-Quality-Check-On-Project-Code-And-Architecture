...
class RefactoredInputValidationMiddleware:
    ...
    async def __call__(self, request: Request, call_next):
        ...
        try:
            ...
            self.blocked_requests += 1
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Request validation failed")
    ...
    def _validate_path(self, path: str) -> None:
        ...
        if self._contains_dangerous_pattern(path):
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid characters in request path")
    ...
    async def _validate_body(self, request: Request) -> None:
        ...
        try:
            ...
            if not validation_result.get("valid", False):
                raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Request body validation failed")
    ...
    def _validate_string(self, value: str, field_name: str) -> None:
        ...
        if len(value) > max_length:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Field '{field_name}' is too long")

        # 检查危险模式
        if self._contains_dangerous_pattern(value):
            logger.warning(f"Dangerous pattern detected in field '{field_name}': {value[:100]}...")
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid characters in field '{field_name}'"
            )
    ...
    def sanitize_html(self, html_content: str) -> str:
        ...
        try:
            ...
            allowed_attributes = {"*": ["class"], "a": ["href", "title"], "img": ["src", "alt", "width", "height"]}
