...
class JavaScriptParser(BaseASTParser):
    ...
    def parse_file(self, file_path: str, content: str | None = None) -> ParsedFile:
        """Parse JavaScript/TypeScript source file"""
        if not self.available:
            return ParsedFile(
                module=ModuleNode(name=Path(file_path).stem, file_path=file_path, language=self.language),
    ...
    def _parse_with_tree_sitter(self, file_path: str, content: str | None = None) -> ParsedFile:
    ...
    def _parse_with_esprima(self, file_path: str, content: str | None = None) -> ParsedFile:
        ...
        if body:
            for child in body.children:
                if child.type == "method_definition":
                    ...
                    if prop_name_node:
                        properties.append(
                            PropertyNode(name=self._get_node_text(prop_name_node, content), is_class_variable=True)
                        )
    ...
    def _parse_class_esprima(self, node: dict) -> ClassNode:
        ...
        for item in body:
            if item.get("type") == "MethodDefinition":
                ...
                prop = PropertyNode(
                    name=item.get("key", {}).get("name", "unknown"), is_class_variable=item.get("static", False)
                )
        ...
        return ClassNode(
            ...
            base_classes=base_classes,
            location=Location(
                file_path="", start_line=loc.get("start", {}).get("line", 1), end_line=loc.get("end", {}).get("line", 1)
            ),
        )
