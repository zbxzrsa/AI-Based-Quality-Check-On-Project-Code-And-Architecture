...
class PythonASTParser(BaseASTParser):
    ...
    def parse_file(self, file_path: str, content: str | None = None) -> ParsedFile:
        ...
        try:
            ...
            return ParsedFile(
                module=ModuleNode(name=Path(file_path).stem, file_path=file_path, language="python"),
    ...
    def _parse_function(self, node, is_method: bool = False) -> FunctionNode:
        ...
        for arg in node.args.args:
            param = ParameterNode(name=arg.arg, type_annotation=ast.unparse(arg.annotation) if arg.annotation else None)
