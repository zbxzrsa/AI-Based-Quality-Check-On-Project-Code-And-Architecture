...
from app.schemas.ast_models import ClassNode, DependencyGraph, FunctionNode, ImportNode, ParsedFile
        ...
        for parsed in parsed_files:
            ...
            for imp in parsed.module.imports:
                ...
                if target in graph.nodes:
                    graph.edges.append(DependencyEdge(source=source, target=target, type="import", weight=1.0))
                    ...
                    for other_parsed in parsed_files:
                        for other_func in other_parsed.module.functions:
                            if call == other_func.name:
                                graph.edges.append(
                                    DependencyEdge(
                                        source=source, target=other_parsed.module.name, type="call", weight=0.5
                                    )
                                )
