...
class GitHubAPIClient:
    ...
    async def get_repository(self, repo_url: str) -> dict[str, Any]:
        ...
        def _get_repo():
            ...
            try:
                ...
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST, detail=f"GitHub API error: {e.data.get('message', str(e))}"
                )
    ...
    async def get_pull_request(self, repo_full_name: str, pr_number: int) -> dict[str, Any]:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"Pull request not found: {e.data.get('message', str(e))}"
            )
    ...
    async def get_file_content(self, repo_full_name: str, file_path: str, ref: str = "main") -> str:
        ...
        try:
            ...
            # Convert client errors to HTTPException
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail=f"File not found: {e.data.get('message', str(e))}"
            )
    ...
    async def update_pr_status(
        ...
        try:
            ...
            status = commit.create_status(state=state, description=description, context=context)
    ...
    async def list_repository_prs(
        ...
        try:
            ...
            # Convert client errors to HTTPException
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=f"Could not list PRs: {e.data.get('message', str(e))}"
            )
    ...
    async def create_webhook(
        self, repo_full_name: str, webhook_url: str, webhook_secret: str, events: list = None
    ) -> dict[str, Any]:
        """
        ...
        def _create_webhook():
            try:
                ...
                return {
                    ...
                    if hasattr(hook.created_at, "isoformat")
                    else str(hook.created_at),
                }
            except GithubException as e:
    ...
    async def list_webhooks(self, repo_full_name: str) -> list[dict[str, Any]]:
        """
