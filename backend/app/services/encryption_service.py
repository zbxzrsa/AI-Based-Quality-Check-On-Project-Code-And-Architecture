...
class EncryptionService:
    ...
    def _get_master_key_from_env(self) -> bytes:
        ...
        if len(key) != self.KEY_SIZE:
            raise ValueError(f"ENCRYPTION_KEY must be {self.KEY_SIZE} bytes when decoded, got {len(key)} bytes")
...
def decrypt_sensitive_field(encrypted_value: str | None) -> str | None:
    """
