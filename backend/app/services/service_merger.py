...
from .service_consolidator import ConsolidationPlan, MergeResult, ServiceFunction
...
class ServiceMerger:
    ...
    async def execute_service_merge(self, plan: ConsolidationPlan) -> MergeResult:
        ...
        try:
            ...
            self._generate_merge_report(plan, merge_ops, reference_updates, validation_results)
    ...
    async def _create_backup(self, plan: ConsolidationPlan) -> Path:
        ...
        config_files = ["docker-compose.yml", "docker-compose.backend.yml", ".env", ".env.example"]
    ...
    async def _plan_merge_operations(self, plan: ConsolidationPlan) -> list[MergeOperation]:
        ...
        for source_service in plan.source_services:
            ...
            if src_dir.exists():
                operations.extend(await self._plan_source_code_migration(source_service, plan.target_service, src_dir))

            # Plan configuration migration
            operations.extend(
                await self._plan_configuration_migration(source_service, plan.target_service, source_path)
            )

            # Plan dependency migration
            operations.extend(await self._plan_dependency_migration(source_service, plan.target_service, source_path))

        return operations

    async def _plan_source_code_migration(
        self, source_service: str, target_service: str, src_dir: Path
    ) -> list[MergeOperation]:
    ...
    async def _plan_configuration_migration(
        self, source_service: str, target_service: str, source_path: Path
    ) -> list[MergeOperation]:
    ...
    async def _plan_dependency_migration(
        self, source_service: str, target_service: str, source_path: Path
    ) -> list[MergeOperation]:
    ...
    async def _find_and_update_references(self, plan: ConsolidationPlan) -> list[ReferenceUpdate]:
        ...
        for search_dir in search_dirs:
            if search_dir.exists():
                reference_updates.extend(await self._search_references_in_directory(search_dir, search_patterns, plan))

        # Search in configuration files
        config_files = ["docker-compose.yml", "docker-compose.backend.yml", ".env", ".env.example", "package.json"]

        for config_file in config_files:
            config_path = self.project_root / config_file
            if config_path.exists():
                reference_updates.extend(await self._search_references_in_file(config_path, search_patterns, plan))
    ...
    async def _search_references_in_directory(
        self, directory: Path, patterns: list[str], plan: ConsolidationPlan
    ...
    async def _search_references_in_file(
        self, file_path: Path, patterns: list[str], plan: ConsolidationPlan
    ) -> list[ReferenceUpdate]:
    ...
    async def _validate_single_function(
        self, function: ServiceFunction, plan: ConsolidationPlan
        ...
        validation = FunctionalityValidation(
            ...
            test_results=[
                ...
                "Function behavior validated",
            ],
        )

        # Simulate some validation issues
        if function.complexity_score > 7:
            validation.issues.append("High complexity function may need additional testing")
    ...
    async def _update_service_registry(self, plan: ConsolidationPlan):
        ...
        if registry_file.exists():
            logger.info(
                f"Would update service registry to remove {plan.source_services} and update {plan.target_service}"
            )

    async def _update_docker_compose(self, plan: ConsolidationPlan):
        """Update Docker Compose configuration"""
        compose_files = ["docker-compose.yml", "docker-compose.backend.yml"]
    ...
    def _generate_merge_report(
        ...
        validations: list[FunctionalityValidation],
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "functionality_validations": [
                {"function_name": val.function_name, "status": val.validation_status, "issues": val.issues}
