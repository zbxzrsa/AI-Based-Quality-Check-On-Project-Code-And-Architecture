...
class GitHubCommentGenerator:
    ...
    def format_security_finding(self, finding: SecurityFinding, include_owasp: bool = True) -> str:
        """
        ...
        if finding.iso_25010_characteristic:
            lines.extend(
                ["### 📋 Standards Classification", f"- **ISO/IEC 25010:** {finding.iso_25010_characteristic}"]
            )
        ...
        # Add footer
        lines.extend(["---", "*Generated by AI Code Review Platform*"])

        return "\n".join(lines)

    def format_classified_finding(self, finding: ClassifiedFinding, include_standards: bool = True) -> str:
        """
        ...
        if finding.owasp_reference:
            lines.extend(["### 🛡️ OWASP Top 10 Reference", f"**{finding.owasp_reference}**", ""])

        # Add standards classification if requested
        if include_standards:
            lines.extend(["### 📋 Standards Classification"])
    ...
    async def post_comment(
        self, repo_full_name: str, pr_number: int, commit_sha: str, file_path: str, line_number: int, comment_body: str
    ) -> CommentResult:
        """
        ...
        try:
            ...
            logger.info(
                ...
                extra={"comment_id": result["id"], "pr_number": pr_number, "file_path": file_path},
            )

            return CommentResult(success=True, comment_id=result["id"])

        except GithubException as e:
            # Check if it's a rate limit error
            if e.status == 403 and "rate limit" in str(e).lower():
                self.rate_limit_hits += 1
                logger.warning("GitHub API rate limit hit, will retry", extra={"pr_number": pr_number})
            ...
            if e.status == 422:
                logger.warning(
                    f"Cannot comment on line {line_number} in {file_path}: line not in diff",
                    extra={"pr_number": pr_number, "file_path": file_path, "line_number": line_number},
                )
                return CommentResult(success=False, error="Line not in PR diff")

            logger.error(
                f"Failed to post comment: {str(e)}",
                extra={"pr_number": pr_number, "file_path": file_path, "error": str(e)},
            )
            ...
            logger.error(
                f"Unexpected error posting comment: {str(e)}", extra={"pr_number": pr_number, "file_path": file_path}
            )

            return CommentResult(success=False, error=str(e))

    async def post_security_findings(
        self, repo_full_name: str, pr_number: int, commit_sha: str, findings: list[SecurityFinding]
    ) -> CommentBatchResult:
        """
        ...
        logger.info(
            ...
            extra={"pr_number": pr_number, "finding_count": len(findings)},
        )
        ...
        for i, finding in enumerate(findings):
            ...
            if not file_path or line_number == 0:
                logger.warning(f"Skipping finding with invalid location: {finding.location}")
                results.append(CommentResult(success=False, error="Invalid location"))
                continue
    ...
    async def post_classified_findings(
        self, repo_full_name: str, pr_number: int, commit_sha: str, findings: list[ClassifiedFinding]
    ) -> CommentBatchResult:
        """
        ...
        for i, finding in enumerate(findings):
            ...
            if not finding.file_path or finding.line_number == 0:
                logger.warning(f"Skipping finding with invalid location: {finding.file_path}:{finding.line_number}")
                results.append(CommentResult(success=False, error="Invalid location"))
                continue
    ...
    async def post_summary_comment(self, repo_full_name: str, pr_number: int, summary: dict[str, Any]) -> CommentResult:
        """
        ...
        try:
            ...
            logger.info(
                f"Posted summary comment to PR #{pr_number}", extra={"comment_id": comment.id, "pr_number": pr_number}
            )

            return CommentResult(success=True, comment_id=comment.id)

        except Exception as e:
            logger.error(f"Failed to post summary comment: {str(e)}", extra={"pr_number": pr_number})
    ...
    def _format_summary(self, summary: dict[str, Any]) -> str:
        ...
        if "security_score" in summary:
            ...
            lines.extend([f"### {score_emoji} Security Score", f"**{score:.1f}/100**", ""])
...
# Singleton instance
_comment_generator_instance: GitHubCommentGenerator | None = None
