...
from app.services.llm.circuit_breaker import CircuitBreaker, CircuitBreakerConfig, CircuitBreakerOpenError
...
class GitHubClientWithCircuitBreaker:
    ...
    def verify_webhook_signature(self, payload_body: bytes, signature_header: str, secret: str) -> bool:
        """
    ...
    async def get_repository(self, repo_url: str) -> dict[str, Any]:
        ...
        try:
            result = await self.circuit_breaker.call_async(self.client.get_repository, repo_url)
            # Cache successful result
            self.cache.set(cache_key, result)
            return result
        except CircuitBreakerOpenError:
            return self._handle_circuit_breaker_open(
                cache_key, f"GitHub API unavailable. Returning cached data for {repo_url} if available."
            )
    ...
    async def get_pull_request(self, repo_full_name: str, pr_number: int) -> dict[str, Any]:
        ...
        try:
            result = await self.circuit_breaker.call_async(self.client.get_pull_request, repo_full_name, pr_number)
            # Cache successful result
            self.cache.set(cache_key, result)
            return result
        except CircuitBreakerOpenError:
            return self._handle_circuit_breaker_open(
                cache_key, f"GitHub API unavailable. Returning cached data for PR #{pr_number} if available."
            )
    ...
    async def get_pr_files(self, repo_full_name: str, pr_number: int) -> list[dict[str, Any]]:
        ...
        try:
            result = await self.circuit_breaker.call_async(self.client.get_pr_files, repo_full_name, pr_number)
            # Cache successful result
            self.cache.set(cache_key, result)
            return result
        except CircuitBreakerOpenError:
            return self._handle_circuit_breaker_open(
                cache_key, "GitHub API unavailable. Returning cached PR files if available."
            )
    ...
    async def get_file_content(self, repo_full_name: str, file_path: str, ref: str = "main") -> str:
        ...
        try:
            result = await self.circuit_breaker.call_async(self.client.get_file_content, repo_full_name, file_path, ref)
            # Cache successful result
            self.cache.set(cache_key, result)
            return result
        except CircuitBreakerOpenError:
            return self._handle_circuit_breaker_open(
                cache_key, "GitHub API unavailable. Returning cached file content if available."
            )

    async def post_review_comment(
        self, repo_full_name: str, pr_number: int, body: str, commit_id: str, path: str, line: int
    ) -> dict[str, Any]:
        """
        ...
        try:
            return await self.circuit_breaker.call_async(
                self.client.post_review_comment, repo_full_name, pr_number, body, commit_id, path, line
            )
        except CircuitBreakerOpenError:
    ...
    async def update_pr_status(
        self, repo_full_name: str, commit_sha: str, state: str, description: str, context: str = "ai-code-review"
    ) -> dict[str, Any]:
        """
        ...
        try:
            return await self.circuit_breaker.call_async(
                self.client.update_pr_status, repo_full_name, commit_sha, state, description, context
            )
        except CircuitBreakerOpenError:
    ...
    async def list_repository_prs(
        self, repo_full_name: str, state: str = "open", limit: int = 30
    ) -> list[dict[str, Any]]:
        """
        ...
        try:
            result = await self.circuit_breaker.call_async(
                self.client.list_repository_prs, repo_full_name, state, limit
            )
            # Cache successful result
            self.cache.set(cache_key, result)
            return result
            ...
            return self._handle_circuit_breaker_open(
                cache_key, "GitHub API unavailable. Returning cached PR list if available."
            )
...
# Singleton instance
_github_client_with_cb: GitHubClientWithCircuitBreaker | None = None
