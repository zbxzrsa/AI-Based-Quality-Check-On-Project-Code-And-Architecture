...
class UserLLMService:
    ...
    async def get_user_llm_provider(
        db: AsyncSession, user_id: str, provider_type: str | None = None, model: str | None = None
    ) -> BaseLLMProvider:
        """
        ...
        if user_api_key:
            ...
            return UserLLMService._create_provider_with_key(provider_type, model, user_api_key)

        # 否则usesystem默认config
        logger.info(
            f"Using system default API key for {provider_type}", extra={"user_id": user_id, "provider": provider_type}
        )
        return UserLLMService._get_system_default_provider(provider_type, model)

    @staticmethod
    def _create_provider_with_key(provider_type: str, model: str, api_key: str) -> BaseLLMProvider:
        """
        ...
        return LLMProviderFactory.create_provider(provider_type=provider_enum, model=model, api_key=api_key)

    @staticmethod
    def _get_system_default_provider(provider_type: str | None = None, model: str | None = None) -> BaseLLMProvider:
        """
        ...
        return LLMProviderFactory.get_provider(provider_type=provider_enum, model=model)

    @staticmethod
    async def get_user_api_settings(db: AsyncSession, user_id: str) -> dict:
        """
        getuser的 API set
