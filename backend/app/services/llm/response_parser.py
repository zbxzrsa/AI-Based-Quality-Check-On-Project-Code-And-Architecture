...
class ResponseParser:
    ...
    LINE_NUMBER_PATTERN = re.compile(r"(?:line|lines?)\s+(\d+)(?:\s*[-–—]\s*(\d+))?", re.IGNORECASE)
    ...
    def __init__(self, default_file_path: str | None = None):
        """
    ...
    def parse(self, response: str, file_path: str | None = None) -> ParseResult:
        """
        ...
        if not response or not response.strip():
            return ParseResult(comments=[], errors=["Empty response from LLM"], raw_response=response, success=False)
        ...
        try:
            ...
            logger.info(f"Parsing complete: {len(comments)} comments, {len(errors)} errors")

            return ParseResult(comments=comments, errors=errors, raw_response=response, success=success)

        except Exception as e:
            error_msg = f"Fatal error parsing response: {str(e)}"
            logger.error(error_msg, exc_info=True)
            return ParseResult(comments=comments, errors=errors + [error_msg], raw_response=response, success=False)
    ...
    def _split_findings(self, response: str) -> list[str]:
        ...
        if len(numbered_findings) > 1:
            ...
            for f in numbered_findings:
                f_stripped = f.strip()
                if f_stripped and (
                    "severity" in f_stripped.lower()
                    or "issue" in f_stripped.lower()
                    or "location" in f_stripped.lower()
                    or len(f_stripped) > 100  # Substantial content
                ):
        ...
        if not findings:
            ...
            if len(markdown_findings) > 1:
                ...
                for f in markdown_findings:
                    f_stripped = f.strip()
                    if f_stripped and (
                        "severity" in f_stripped.lower() or "issue" in f_stripped.lower() or len(f_stripped) > 100
                    ):
                        findings.append(f_stripped)
        ...
        if not findings:
            severity_findings = re.split(r"\n\s*(?=(?:severity|level)\s*[:：])", response, flags=re.IGNORECASE)
        ...
        findings = [
            f
            for f in findings
            if len(f) > 20 and ("severity" in f.lower() or "issue" in f.lower() or "problem" in f.lower())
        ]
    ...
    def _parse_finding(self, finding: str, default_file_path: str) -> ReviewComment | None:
        ...
        file_path, line_start, line_end = self._extract_location(finding, default_file_path)
    ...
    def _extract_location(self, text: str, default_file_path: str) -> tuple[str, int | None, int | None]:
        """
        ...
        location_match = re.search(r"(?:location|file|path)\s*[:：]\s*([^\n]+)", text, re.IGNORECASE)
        ...
        if location_match:
            ...
            if path_part and path_part.lower() not in ["n/a", "none", "unknown", "n", "a"]:
...
def parse_llm_response(response: str, file_path: str | None = None) -> ParseResult:
    """
