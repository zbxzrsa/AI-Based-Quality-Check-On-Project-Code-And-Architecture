...
class LLMOrchestrator:
    ...
    def __init__(
        ...
        self.primary_provider = self._create_provider(
            self.config.primary_provider, self.config.primary_model, primary_api_key, self.config.timeout
        )

        self.fallback_provider = self._create_provider(
            self.config.fallback_provider, self.config.fallback_model, fallback_api_key, self.config.timeout
        )

        # Create circuit breakers
        cb_config = self.config.circuit_breaker_config or CircuitBreakerConfig()
        self.primary_circuit = CircuitBreaker(f"{self.config.primary_provider.value}_circuit", cb_config)
        self.fallback_circuit = CircuitBreaker(f"{self.config.fallback_provider.value}_circuit", cb_config)
    ...
    def _create_provider(
        self, provider_type: LLMProviderType, model: str | None, api_key: str | None, timeout: int
    ) -> BaseLLMProvider:
        ...
        return LLMProviderFactory.create_provider(provider_type, model=model, **kwargs)

    async def generate(self, request: LLMRequest, use_fallback: bool = True) -> LLMResponse:
        """
        ...
        try:
            response = await self.primary_circuit.call_async(self.primary_provider.generate, request)
            ...
            logger.warning(
                f"Primary provider circuit is OPEN: {str(e)}", extra={"provider": self.config.primary_provider.value}
            )
        ...
        if use_fallback:
            try:
                response = await self.fallback_circuit.call_async(self.fallback_provider.generate, request)
                ...
                raise Exception("Both primary and fallback providers unavailable (circuits open)")
                ...
                raise Exception(f"Both primary and fallback providers failed. Last error: {str(e)}")
