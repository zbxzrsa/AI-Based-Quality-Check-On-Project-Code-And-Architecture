...
class LLMProviderFactory:
    ...
    @classmethod
    def create_provider(
        cls, provider_type: LLMProviderType, model: str | None = None, api_key: str | None = None, **kwargs
    ) -> BaseLLMProvider:
        """
        ...
        try:
            ...
            logger.error(
                f"Failed to create {provider_name} provider: {str(e)}", extra={"provider": provider_name}, exc_info=True
            )
            raise

    @classmethod
    def _create_openai_provider(cls, model: str | None = None, api_key: str | None = None, **kwargs) -> OpenAIProvider:
        ...
        logger.info(f"Creating OpenAI provider with model: {model}", extra={"provider": "openai", "model": model})

        return OpenAIProvider(
            model=model, api_key=api_key, base_url=kwargs.get("base_url"), timeout=kwargs.get("timeout", 30)
        )

    @classmethod
    def _create_anthropic_provider(
        cls, model: str | None = None, api_key: str | None = None, **kwargs
    ) -> AnthropicProvider:
        ...
        logger.info(f"Creating Anthropic provider with model: {model}", extra={"provider": "anthropic", "model": model})

        return AnthropicProvider(model=model, api_key=api_key, timeout=kwargs.get("timeout", 30))
    ...
    def _create_openrouter_provider(
        ...
        logger.info(
            f"Creating OpenRouter provider with model: {model}", extra={"provider": "openrouter", "model": model}
        )

        return OpenRouterProvider(
            model=model, api_key=api_key, base_url=settings.OPENROUTER_BASE_URL, timeout=kwargs.get("timeout", 60)
        )

    @classmethod
    def _create_lmstudio_provider(cls, model: str | None = None, **kwargs) -> LMStudioProvider:
    ...
    @classmethod
    def get_provider(
        cls, provider_type: LLMProviderType, model: str | None = None, use_cache: bool = True
    ) -> BaseLLMProvider:
        """
        ...
        if use_cache and cache_key in cls._instances:
            logger.debug(
                f"Returning cached provider: {cache_key}", extra={"provider": provider_type.value, "model": model}
            )
        ...
        if use_cache:
            ...
            logger.debug(f"Cached provider: {cache_key}", extra={"provider": provider_type.value, "model": model})
...
def get_llm_provider(
    provider_type: LLMProviderType = LLMProviderType.OPENAI, model: str | None = None
) -> BaseLLMProvider:
    """
...
def get_default_llm_provider(model: str | None = None) -> BaseLLMProvider:
    """
