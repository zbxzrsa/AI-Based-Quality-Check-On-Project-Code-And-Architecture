...
from app.models import InvitationStatus, Project, ProjectInvitation, ProjectMember, ProjectRole, User
...
class ProjectInvitationService:
    ...
    async def create_invitation(
        ...
        existing_member = await ProjectInvitationService.get_project_member(db, project_id, invitee_email)
        if existing_member:
        ...
        await AuditService.log_project_invitation_sent(
            db=db, project_id=project_id, inviter_id=inviter_id, invitee_email=invitee_email, role=role
        )

        return invitation

    @staticmethod
    async def accept_invitation(db: AsyncSession, invitation_token: str, user_id: uuid.UUID) -> ProjectMember:
        """
        ...
        result = await db.execute(
            ...
            .where(ProjectInvitation.invitation_token == invitation_token)
        ...
        existing_member = await db.execute(
            select(ProjectMember).where(
                and_(ProjectMember.project_id == invitation.project_id, ProjectMember.user_id == user_id)
            )
        )
        ...
        await AuditService.log_project_invitation_accepted(
            db=db, project_id=invitation.project_id, user_id=user_id, role=invitation.role
        )

        return member

    @staticmethod
    async def decline_invitation(db: AsyncSession, invitation_token: str, user_id: uuid.UUID) -> ProjectInvitation:
        """
        ...
        # Get invitation
        result = await db.execute(
            select(ProjectInvitation).where(ProjectInvitation.invitation_token == invitation_token)
        )
        invitation = result.scalar_one_or_none()
    ...
    @staticmethod
    async def get_project_member(db: AsyncSession, project_id: uuid.UUID, user_email: str) -> ProjectMember | None:
        """Get project member by email"""
        result = await db.execute(
            select(ProjectMember)
            .join(User, ProjectMember.user_id == User.id)
            .where(and_(ProjectMember.project_id == project_id, User.email == user_email))
        )
        return result.scalar_one_or_none()

    @staticmethod
    async def get_user_project_memberships(db: AsyncSession, user_id: uuid.UUID) -> list[ProjectMember]:
        """Get all project memberships for a user"""
        result = await db.execute(
            select(ProjectMember).where(ProjectMember.user_id == user_id).options(selectinload(ProjectMember.project))
        )
        return result.scalars().all()

    @staticmethod
    async def get_project_members(db: AsyncSession, project_id: uuid.UUID) -> list[ProjectMember]:
    ...
    @staticmethod
    async def get_pending_invitations_for_user(db: AsyncSession, user_email: str) -> list[ProjectInvitation]:
        ...
        result = await db.execute(
            ...
            .options(selectinload(ProjectInvitation.project), selectinload(ProjectInvitation.inviter))
        )
        return result.scalars().all()
    ...
    @staticmethod
    async def has_project_access(db: AsyncSession, user_id: uuid.UUID, project_id: uuid.UUID) -> bool:
        """Check if user has access to a project"""
        # Check if user is project owner
        project_result = await db.execute(
            select(Project).where(and_(Project.id == project_id, Project.owner_id == user_id))
        )
