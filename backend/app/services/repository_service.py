...
class RepositoryService:
    ...
    def parse_repository_url(self, url: str) -> RepositoryInfo:
        ...
        return RepositoryInfo(owner=owner, name=name, url_format=url_format, full_url=url, clone_url=clone_url)

    async def validate_repository(
        self, repo_info: RepositoryInfo, branch: str | None = None
    ) -> RepositoryValidationResult:
        """
        ...
        try:
                ...
                async with session.get(repo_url, headers=headers) as response:
                    if response.status == 404:
                        return RepositoryValidationResult(
                            is_valid=False, is_accessible=False, exists=False, error_message="Repository not found"
                        )
            ...
            return RepositoryValidationResult(
                is_valid=False, is_accessible=False, exists=False, error_message=f"Network error: {str(e)}"
            )
        except Exception as e:
            logger.error(f"Unexpected error validating repository: {e}")
            return RepositoryValidationResult(
                is_valid=False, is_accessible=False, exists=False, error_message=f"Validation error: {str(e)}"
            )

    async def fetch_dependencies(self, repo_info: RepositoryInfo, branch: str = "main") -> DependencyInfo | None:
        """
    ...
    async def add_repository(self, request: AddRepositoryRequest, user_id: str) -> RepositoryResponse:
        """
        ...
        logger.info(f"Repository added: {repo_info.owner}/{repo_info.name} (branch: {branch}, user: {user_id})")
