...
class AIReasoningEngine:
    ...
    def __init__(self, provider: LLMProvider = LLMProvider.OPENAI, model: str | None = None):
    ...
    async def analyze_pull_request(
        ...
        try:
            response = await self.llm_client.generate_completion(
                system_prompt=system_prompt, user_prompt=user_prompt, temperature=0.3, max_tokens=4000, json_mode=True
            )
    ...
    async def assemble_context(self, project_id: str, pr_id: str) -> dict[str, Any]:
        """
        ...
        try:
            ...
            dependency_sections.append(
                f"Project contains {graph['metadata']['node_count']} architectural components with {graph['metadata']['edge_count']} dependencies."
            )
            ...
            if cycles:
                ...
                for cycle in cycles[:5]:  # Limit to first 5 cycles
                    cycle_details.append(
                        f"Cycle length {cycle['cycleLength']}: {' -> '.join(cycle['cyclePath'][:5])}..."
                    )
                dependency_sections.append(
                    f"⚠️  WARNING: {len(cycles)} circular dependencies detected. Examples: {'; '.join(cycle_details)}"
                )
            ...
            if metrics.get("coupling_metrics"):
                ...
                if unstable_modules:
                    ...
                    dependency_sections.append(
                        f"⚠️  High instability modules (instability > 0.8): {', '.join(unstable_names)}"
                    )

            context["complexity_summary"] = (
                f"Average complexity: {metrics['complexity_metrics']['average_complexity']:.1f}"
            )
            ...
            context["dependency_graph_summary"] = (
                "Unable to fetch dependency graph from Neo4j. Analysis will proceed without architectural context."
            )
