...
from app.core.prometheus_metrics import record_health_check, set_dependency_status
...
class HealthCheckResponse:
    ...
    def to_dict(self):
        ...
        return {
            ...
            "databases": {name: asdict(db) for name, db in self.databases.items()},
...
class HealthService:
    ...
    async def get_health_status(self) -> HealthCheckResponse:
        ...
        try:
            db_status = await asyncio.wait_for(self.connection_manager.verify_all(), timeout=10.0)
        ...
        if "PostgreSQL" in db_status:
            ...
            databases["PostgreSQL"] = DatabaseHealth(
                name="PostgreSQL", is_connected=False, error="Health check timeout or unavailable"
            )
        ...
        for service_name in ["Neo4j", "Redis"]:
            if service_name in db_status:
                ...
                databases[service_name] = DatabaseHealth(
                    name=service_name, is_connected=False, error="Service unavailable"
                )
                # Record dependency status metric
        ...
        if settings.LLM_ENABLED:
            ...
            services["LLM"] = ServiceHealth(
                name="LLM", is_available=llm_available, error=None if llm_available else "LLM service unavailable"
            )
    ...
    async def check_neo4j(self) -> DatabaseHealth:
        ...
        return DatabaseHealth(
            name="Neo4j", is_connected=status.is_connected, response_time_ms=status.response_time_ms, error=status.error
        )
    ...
    async def check_redis(self) -> DatabaseHealth:
        ...
        return DatabaseHealth(
            name="Redis", is_connected=status.is_connected, response_time_ms=status.response_time_ms, error=status.error
        )
        ...
        # Check service dependencies
            ...
            if not settings.is_celery_enabled():
                result = DatabaseHealth(
                    name=name, is_connected=False, response_time_ms=0.0, error="Celery is not enabled"
                )
            else:
                celery_available = await self._check_celery_health()
                result = DatabaseHealth(
            ...
            if not settings.LLM_ENABLED:
                result = DatabaseHealth(
                    name=name, is_connected=False, response_time_ms=0.0, error="LLM service is not enabled"
                )
            else:
                llm_available = await self._check_llm_health()
                result = DatabaseHealth(
