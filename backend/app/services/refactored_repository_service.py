...
from app.core.states import state_machine_factory
...
class RefactoredRepositoryService(
    BaseCRUDService[Repository, AddRepositoryRequest, RepositoryUpdateRequest, RepositoryResponse]
):
    """
    重构后的Repository服务
    ...
    def __init__(self, db: AsyncSession, github_token: str | None = None):
    ...
    async def validate_create_data(self, create_data: AddRepositoryRequest, user_id: str) -> None:
        ...
        validation_result = await self.github_processor.validate_connection(connection_type, connection_config)

        if not validation_result["valid"]:
            raise ValidationError(validation_result["error"])

        # 验证仓库可访问性
        repo_validation = await self.validator.validate_repository(repo_info.__dict__, create_data.branch)
    ...
    async def validate_update_data(
        self, update_data: RepositoryUpdateRequest, entity: Repository, user_id: str
    ) -> None:
        """验证更新数据"""
        # 基本验证 - 可以根据需要扩展
    ...
    async def preprocess_update_data(
        self, update_dict: dict[str, Any], entity: Repository, user_id: str, **kwargs
    ) -> dict[str, Any]:
        """预处理更新数据"""
        # 如果更新了仓库URL，需要重新验证
        if "repository_url" in update_dict:
            repo_info = self.parse_repository_url(update_dict["repository_url"])
            update_dict.update({"owner": repo_info.owner, "name": repo_info.name, "clone_url": repo_info.clone_url})
    ...
    async def validate_repository_access(self, repo_info: RepositoryInfo, branch: str | None = None) -> dict[str, Any]:
        """
        验证仓库访问 - 使用模板方法模式
    ...
    async def fetch_dependencies(self, repo_info: RepositoryInfo, branch: str = "main") -> Any | None:
        """
        获取依赖信息 - 保持原有逻辑
        ...
        try:
            async with aiohttp.ClientSession() as session:
                # 尝试获取package.json
                package_json_url = (
                    f"{api_base}/repos/{repo_info.owner}/{repo_info.name}/contents/package.json?ref={branch}"
                )
                ...
                requirements_url = (
                    f"{api_base}/repos/{repo_info.owner}/{repo_info.name}/contents/requirements.txt?ref={branch}"
                )
                ...
                async with session.get(requirements_url, headers=headers) as response:
                    if response.status == 200:
                        ...
                        return DependencyInfo(
                            package_manager="pip", dependencies=dependencies, dev_dependencies={}, peer_dependencies={}
                        )
            ...
            if state_machine.get_current_state_name() == "active":
                ...
                if result["success"]:
                    ...
                    if validation_result["is_valid"]:
                        ...
                        return {"success": True, "message": "Repository synced successfully", "repository": repository}
                    else:
                        # 验证失败，转换到失败状态
                        await state_machine.transition_to(
                            "failed", error=validation_result.get("error_message"), user_id=user_id
                        )
                        return {
                            "success": False,
    ...
    async def archive_repository(self, repository_id: str, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if result["success"]:
                return {"success": True, "message": "Repository archived successfully", "repository": repository}
            else:
                return result
    ...
    async def reactivate_repository(self, repository_id: str, user_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if current_state == "archived":
                ...
                return {"success": False, "error": f"Repository is in {current_state} state and cannot be reactivated"}

            if result["success"]:
                return {"success": True, "message": "Repository reactivation started", "repository": repository}
            else:
                return result
...
def create_repository_service(db: AsyncSession, github_token: str | None = None) -> RefactoredRepositoryService:
    """
    创建Repository服务实例
