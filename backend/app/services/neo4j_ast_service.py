...
class Neo4jASTService(BaseNeo4jService):
    ...
    async def _insert_class(self, session, cls: ClassNode, project_id: str, file_path: str):
        """Insert a class node"""
    ...
    async def _insert_function(
        self, session, func: FunctionNode, project_id: str, file_path: str, class_id: str | None
    ):
    ...
    async def detect_drift(self, project_id: str, baseline_version: str) -> dict[str, Any]:
        """
    ...
    async def get_dependency_graph(self, project_id: str) -> dict[str, Any]:
        ...
        async with self.driver.session(database=settings.NEO4J_DATABASE) as session:
            ...
            return {
                ...
                "metadata": {"node_count": len(nodes), "edge_count": len(edges), "project_id": project_id},
            }
    ...
    async def detect_cyclic_dependencies(self, project_id: str) -> list[dict[str, Any]]:
        """
        ...
        results = await self.run_query(cypher_queries.CYCLIC_DEPENDENCY_QUERY, projectId=project_id)
    ...
    async def detect_layer_violations(
        ...
        results = await self.run_query(cypher_queries.LAYER_VIOLATION_QUERY, projectId=project_id)
    ...
    async def detect_direct_cycles(self, project_id: str) -> list[dict[str, Any]]:
        ...
        results = await self.run_query(cypher_queries.DIRECT_CYCLES_QUERY, projectId=project_id)
    ...
    async def find_cyclic_services(self, project_id: str) -> list[dict[str, Any]]:
        ...
        results = await self.run_query(cypher_queries.CYCLIC_SERVICE_QUERY, projectId=project_id)

        return results

    async def detect_all_layer_violations(self, project_id: str) -> list[dict[str, Any]]:
        """
        Detect all layer violations (not just Controller->Repository)
        """
        from app.services import cypher_queries

        results = await self.run_query(cypher_queries.ALL_LAYER_VIOLATIONS_QUERY, projectId=project_id)
    ...
    async def calculate_coupling_metrics(self, project_id: str) -> dict[str, Any]:
        ...
        efferent_results = await self.run_query(cypher_queries.EFFERENT_COUPLING_QUERY, projectId=project_id)

        # Get afferent coupling
        afferent_results = await self.run_query(cypher_queries.AFFERENT_COUPLING_QUERY, projectId=project_id)

        # Get instability index
        instability_results = await self.run_query(cypher_queries.INSTABILITY_INDEX_QUERY, projectId=project_id)
    ...
    async def find_longest_dependency_paths(self, project_id: str, limit: int = 20) -> list[dict[str, Any]]:
        ...
        results = await self.run_query(cypher_queries.LONGEST_DEPENDENCY_PATHS_QUERY, projectId=project_id)
        ...
        query = cypher_queries.LONGEST_DEPENDENCY_PATHS_QUERY.replace("LIMIT 20", f"LIMIT {limit}")

        results = await self.run_query(query, projectId=project_id)
        return results

    async def generate_weekly_drift_report(self, project_id: str) -> dict[str, Any]:
        """
        Generate comprehensive weekly drift report
        """
        from app.services import cypher_queries

        results = await self.run_query(cypher_queries.WEEKLY_DRIFT_REPORT_QUERY, projectId=project_id)

        if results:
            return dict(results[0])

        return {"cycle_count": 0, "violation_count": 0, "average_instability": 0.0, "unstable_modules": 0}

    async def run_query(self, query: str, **parameters) -> list[dict[str, Any]]:
        """
    ...
    def _calculate_drift_score(self, cycles: list[dict], violations: list[dict], metrics: dict) -> float:
        """
        ...
        high_instability = sum(1 for m in unstable_modules if m.get("instability_index", 0) > 0.7)
