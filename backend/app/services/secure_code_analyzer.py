...
class SecureASTVisitor(ast.NodeVisitor):
    ...
    def __init__(self, filename: str = "<string>"):
        ...
        self.dangerous_imports = {"subprocess", "pickle", "os", "sys", "shutil"}
    ...
    def _contains_hardcoded_secret(self, text: str) -> bool:
        ...
        secret_indicators = ["password", "secret", "key", "token", "api_key", "sk-", "pk-", "Bearer ", "Authorization:"]
    ...
    def _is_suspicious_function_name(self, name: str) -> bool:
        ...
        suspicious_patterns = ["inject", "exploit", "hack", "bypass", "override", "unsafe", "dangerous", "evil"]
...
class SecureCodeAnalyzer:
    ...
    def get_summary_report(self, results: dict[str, AnalysisResult]) -> dict[str, Any]:
        ...
        severity_counts = {AnalysisRisk.CRITICAL: 0, AnalysisRisk.HIGH: 0, AnalysisRisk.MEDIUM: 0, AnalysisRisk.LOW: 0}
...
"""
    ...
    logger.info(f"Functions: {result.functions_count}, Classes: {result.classes_count}")

    for issue in result.issues:
        logger.info(f"[{issue.severity.value.upper()}] {issue.issue_type}: {issue.description}")
        if issue.suggestion:
