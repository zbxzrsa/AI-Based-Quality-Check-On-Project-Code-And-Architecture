...
class CodeReviewer:
    ...
    async def review_pull_request(
        ...
        review_result = CodeReviewResult(
            pr_id=pr_data["id"], project_id=project_id, commit_sha=pr_data["head_sha"], comments=[]
        )

        # Analyze each changed file
        tasks = []
        for file_change in file_changes:
            task = self._analyze_file_changes(file_change=file_change, pr_data=pr_data, project_id=project_id)
            tasks.append(task)
    ...
    async def _analyze_file_changes(self, file_change: dict, pr_data: dict, project_id: str) -> CodeReviewResult:
        """Analyze changes in a single file"""
        file_review = CodeReviewResult(pr_id=pr_data["id"], project_id=project_id, file_path=file_change["file_path"])
        ...
        try:
            ...
            prompt = self._create_review_prompt(file_change=file_change, ast_info=ast_info, pr_data=pr_data)
            ...
            if self.agentic_ai_service and file_change.get("content"):
                try:
                    ...
                    if ai_comments:
                        ...
                        logger.info(f"Added {len(ai_comments)} AI-generated comments for {file_change['file_path']}")
                except Exception as e:
                    logger.error(f"Error querying Agentic AI Service for {file_change['file_path']}: {str(e)}")
    ...
    async def _query_agentic_ai_for_complex_analysis(
        self, file_path: str, code_content: str, repository: str, existing_comments: list[ReviewComment]
    ) -> list[ReviewComment]:
        """
        ...
        try:
            ...
            clean_code_violations = await self.agentic_ai_service.analyze_clean_code_violations(
                code=code_content, file_path=file_path, language=self._detect_language(file_path)
            )
            ...
            for violation in clean_code_violations:
                ...
                logger.debug(f"Added Clean Code violation: {violation.principle.value} at {violation.location}")
            ...
            architectural_analysis = await self.agentic_ai_service.analyze_with_graph_context(
                code=code_content, file_path=file_path, repository=repository
            )
            ...
            if complex_findings:
                logger.info(f"Generating natural language explanations for {len(complex_findings)} complex findings")
                ...
                for finding in complex_findings[:3]:  # Limit to top 3 to avoid excessive API calls
                    try:
                        ...
                        logger.debug(f"Generated natural language explanation for finding at line {finding.line}")

                    except Exception as e:
                        logger.warning(f"Failed to generate explanation for finding at line {finding.line}: {e}")
            ...
            logger.error(f"Error during Agentic AI analysis for {file_path}: {str(e)}", exc_info=True)
    ...
    async def _perform_architectural_analysis(self, review_result: CodeReviewResult, project_id: str) -> None:
        """Perform architectural analysis on the codebase"""
        try:
            # Get architectural dependencies
            await self.arch_analyzer.analyze_dependencies(project_id)

            # Check for architectural violations
            violations = await self.arch_analyzer.detect_violations(
                project_id=project_id, changes=review_result.comments
            )
