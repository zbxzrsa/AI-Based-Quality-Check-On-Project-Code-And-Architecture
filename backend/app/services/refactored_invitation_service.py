...
class InvitationCreationProcess(BusinessProcessTemplate):
    ...
    async def _check_existing_member(self, db: AsyncSession, project_id: UUID, email: str) -> bool:
        ...
        result = await db.execute(
            ...
            .where(and_(ProjectMember.project_id == project_id, User.email == email))
        )
...
class InvitationResponseProcess(BusinessProcessTemplate):
    ...
    async def execute_business_logic(
        ...
        try:
            ...
            # 根据操作类型处理
            if action == "accept":
                result = await state_machine.handle_action("accept", user_id=UUID(user_id), user_email=user_email)
            elif action == "decline":
                result = await state_machine.handle_action("decline", user_id=UUID(user_id), user_email=user_email)
            else:
...
class RefactoredInvitationService:
    ...
    async def create_invitation(
        ...
        result = await self.creation_process.execute_process(db=self.db, input_data=input_data, user_id=inviter_id)
        ...
        if result["success"]:
            ...
            await UnifiedAuditService.log_action(
                ...
                success=True,
                changes={"project_id": project_id, "invitee_email": invitee_email, "role": role},
            )
    ...
    async def accept_invitation(self, invitation_token: str, user_id: str) -> dict[str, Any]:
        ...
        """
        # 获取用户邮箱
        user_result = await self.db.execute(select(User).where(User.id == UUID(user_id)))
        user = user_result.scalar_one_or_none()
        user_email = user.email if user else None

        input_data = {"invitation_token": invitation_token, "action": "accept"}

        context = {"user_email": user_email}

        return await self.response_process.execute_process(
            db=self.db, input_data=input_data, user_id=user_id, context=context
        )

    async def decline_invitation(self, invitation_token: str, user_id: str) -> dict[str, Any]:
        """
        拒绝邀请 - 使用状态机处理
        ...
        input_data = {"invitation_token": invitation_token, "action": "decline"}
    ...
    async def has_project_access(self, user_id: str, project_id: str) -> bool:
        ...
        try:
            ...
            project_result = await self.db.execute(
                select(Project).where(and_(Project.id == project_uuid, Project.owner_id == user_uuid))
            )
            ...
            member_result = await self.db.execute(
                select(ProjectMember).where(
                    and_(ProjectMember.project_id == project_uuid, ProjectMember.user_id == user_uuid)
                )
            )
