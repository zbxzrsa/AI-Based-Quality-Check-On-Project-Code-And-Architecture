...
class SlowQueryMonitor:
    ...
    def get_slow_query_stats(self) -> dict[str, Any]:
        ...
        if not self.slow_queries:
            return {"total_slow_queries": 0, "avg_query_time_ms": 0, "max_query_time_ms": 0, "min_query_time_ms": 0}
    ...
    def clear_old_entries(self, hours: int = 24):
        ...
        self.slow_queries = [q for q in self.slow_queries if datetime.fromisoformat(q["timestamp"]) > cutoff_time]
...
def analyze_slow_queries(db: AsyncSession) -> dict[str, Any]:
    ...
    if not slow_queries:
        return {"status": "no_slow_queries", "message": "No slow queries detected", "recommendations": []}
