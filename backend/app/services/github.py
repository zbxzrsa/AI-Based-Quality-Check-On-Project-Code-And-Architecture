...
class GitHubService:
    ...
    def __init__(self, access_token: str | None = None, api_url: str = "https://api.github.com"):
        """
    ...
    async def get_pull_request_diff(
        self, owner: str, repo: str, pull_number: int, media_type: str = "diff"
    ) -> str | None:
        """
        ...
        try:
            async with aiohttp.ClientSession(headers=self.headers) as session:
                async with session.get(
                    urljoin(self.base_url, url), headers={"Accept": f"application/vnd.github.v3.{media_type}"}
                ) as response:
    ...
    async def get_repository_content(
        ...
        try:
            async with aiohttp.ClientSession(headers=self.headers) as session:
                async with session.get(urljoin(self.base_url, url), params=params) as response:
                    if response.status == 200:
                        return await response.json()
                    else:
                        await response.text()
