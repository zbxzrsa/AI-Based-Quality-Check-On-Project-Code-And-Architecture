...
from .models import GraphUpdateResult
...
class GraphBuilderService:
    ...
    async def create_or_update_entity_node(
        self, entity: CodeEntity, project_id: str | None = None
    ) -> GraphUpdateResult:
        """
    ...
    async def create_or_update_entity_nodes_batch(
        self, entities: list[CodeEntity], project_id: str | None = None
    ) -> GraphUpdateResult:
        """
                        ...
                        try:
                            try:
                                query_result = await tx.run(
                                    query, batch=batch_data, timestamp=datetime.utcnow().isoformat()
                                )
                            except Exception:
                                # Fallback if APOC is not available
                                query_result = await tx.run(
                                    fallback_query, batch=batch_data, timestamp=datetime.utcnow().isoformat()
                                )
    ...
    async def create_dependency_relationship(
        ...
        try:
            ...
            result.errors.append(f"Error creating relationship {source_entity.name} -> {target_entity.name}: {str(e)}")

        return result

    async def create_dependency_relationships_batch(
        self, relationships: list[tuple[CodeEntity, CodeEntity, str, dict | None]], project_id: str | None = None
    ) -> GraphUpdateResult:
        """
    ...
    async def build_dependency_graph_from_entities(
        self, entities: list[CodeEntity], dependency_graph: DependencyGraph, project_id: str | None = None
    ) -> GraphUpdateResult:
        """
        ...
        for edge in dependency_graph.edges:
            ...
            if source and target:
                ...
                properties = {"weight": edge.weight, "edge_type": edge.type}
    ...
    async def update_entity_metrics(
        self, entity_name: str, file_path: str, metrics: dict[str, Any]
    ) -> GraphUpdateResult:
        """
    ...
    async def delete_entities_by_file(self, file_path: str, project_id: str | None = None) -> GraphUpdateResult:
        """
    ...
    async def get_entity_dependencies(self, entity_name: str, file_path: str, depth: int = 1) -> list[dict[str, Any]]:
        """
        ...
        try:
            async with driver.session(database=settings.NEO4J_DATABASE) as session:
                ...
                query_result = await session.run(query, name=entity_name, file_path=file_path)
    ...
    def _get_node_label(self, entity_type: str) -> str:
        ...
        Returns:
            Neo4j node label
        """
        label_map = {"function": "Function", "class": "Class", "method": "Method", "module": "Module", "file": "File"}
    ...
    def _map_edge_type_to_relationship(self, edge_type: str) -> str:
        ...
        """
        rel_map = {"import": "IMPORTS", "call": "CALLS", "inheritance": "INHERITS", "uses": "USES"}
