...
class CircularDependencyDetector:
    ...
    async def detect_cycles(
        self, project_id: str | None = None, min_severity: CycleSeverity | None = None
    ...
    async def detect_cycles_for_entity(
        self, entity_name: str, file_path: str, project_id: str | None = None
        ...
        entity_cycles = [cycle for cycle in result.cycles if entity_name in cycle.nodes]

        return entity_cycles

    async def get_cycle_visualization_data(self, cycle_id: str, project_id: str | None = None) -> dict[str, Any] | None:
        """
        ...
        for source, target in cycle.edges:
            edges.append({"source": source, "target": target, "type": "DEPENDS_ON"})
    ...
    async def _find_strongly_connected_components(self, project_id: str | None = None) -> list[list[str]]:
        """
    ...
    async def _build_cycle_object(
        self, cycle_nodes: list[str], project_id: str | None = None
    ) -> CircularDependency | None:
        """
        ...
        try:
            async with driver.session(database=settings.NEO4J_DATABASE) as session:
                ...
                async for record in query_result:
                    name = record["name"]
                    nodes_data[name] = {"file_path": record["file_path"] or "", "complexity": record["complexity"] or 1}
    ...
    def _calculate_severity(self, depth: int, total_complexity: int, avg_complexity: float) -> CycleSeverity:
        """
        Calculate cycle severity based on depth and complexity
