...
class BaselineManager:
    ...
    @staticmethod
    def _is_valid_baseline_id(baseline_id: str) -> bool:
        """Allow only baseline identifiers generated by this service."""
        return bool(re.fullmatch(r"[A-Za-z0-9_.-]+", baseline_id))
    ...
    async def _load_baseline_by_id(self, baseline_id: str) -> ArchitectureBaseline | None:
        ...
        Returns:
            ArchitectureBaseline or None if not found
        """
        # Validate baseline_id to prevent path traversal and unsafe names
        if not baseline_id or not self._is_valid_baseline_id(baseline_id):
            logger.warning("Invalid baseline_id rejected")
            return None

        safe_storage_path = self.storage_path.resolve()
        baseline_file = (safe_storage_path / f"{baseline_id}.json").resolve()
        if safe_storage_path not in baseline_file.parents:
            logger.warning("Baseline path traversal attempt rejected")
            return None
        ...
        try:
            ...
            logger.info("Error loading baseline", extra={"baseline_id": self._sanitize_log_input(baseline_id)})
            return None
