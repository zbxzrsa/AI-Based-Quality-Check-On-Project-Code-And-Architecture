...
class DriftDetector:
    ...
    def __init__(self, baseline_manager: BaselineManager | None = None):
        """
    ...
    async def detect_drift(self, project_id: str, baseline_version: str | None = None) -> DriftResult:
        """
        ...
        """
        # Get baseline
        baseline = await self.baseline_manager.get_baseline(project_id=project_id, version=baseline_version)
        ...
        rel_changes = self._compare_relationships(baseline.relationships, current_relationships)
        drift_details.extend(rel_changes)

        # Calculate metrics
        metrics = self._calculate_drift_metrics(baseline, current_nodes, current_relationships, drift_details)
    ...
    async def _get_current_nodes(self, driver: Any, project_id: str) -> list[dict[str, Any]]:
    ...
    async def _get_current_relationships(self, driver: Any, project_id: str) -> list[dict[str, Any]]:
    ...
    def _compare_nodes(
        self, baseline_nodes: list[dict[str, Any]], current_nodes: list[dict[str, Any]]
    ) -> list[DriftDetail]:
        """
        ...
        baseline_map = {(n["properties"].get("name"), n["properties"].get("file_path")): n for n in baseline_nodes}
        current_map = {(n["properties"].get("name"), n["properties"].get("file_path")): n for n in current_nodes}
    ...
    def _compare_relationships(
        self, baseline_rels: list[dict[str, Any]], current_rels: list[dict[str, Any]]
    ) -> list[DriftDetail]:
        """
        ...
        baseline_map = {(r["source_name"], r["target_name"], r["rel_type"]): r for r in baseline_rels}
        current_map = {(r["source_name"], r["target_name"], r["rel_type"]): r for r in current_rels}
        ...
        for key in new_keys:
            ...
            details.append(
                DriftDetail(
                    ...
                    details={"source": source, "target": target, "relationship_type": rel_type},
    ...
    def _calculate_drift_metrics(
        ...
        new_nodes = sum(1 for d in drift_details if d.change_type == "added" and d.entity_type == "node")
        removed_nodes = sum(1 for d in drift_details if d.change_type == "removed" and d.entity_type == "node")
        modified_nodes = sum(1 for d in drift_details if d.change_type == "modified" and d.entity_type == "node")

        new_dependencies = sum(1 for d in drift_details if d.change_type == "added" and d.entity_type == "relationship")
        removed_dependencies = sum(
            1 for d in drift_details if d.change_type == "removed" and d.entity_type == "relationship"
        )
        modified_relationships = sum(
            1 for d in drift_details if d.change_type == "modified" and d.entity_type == "relationship"
        )
        ...
        len(current_nodes) + len(current_relationships)
        ...
        current_complexity = sum(n["properties"].get("complexity", 0) for n in current_nodes)
        complexity_delta = current_complexity - baseline_complexity
    ...
    def _generate_summary(self, metrics: DriftMetrics, severity: DriftSeverity) -> str:
        """
        ...
        if metrics.complexity_delta != 0:
            ...
            summary_parts.append(f"Overall complexity {direction} by {abs(metrics.complexity_delta)}.")

        return " ".join(summary_parts)

    def _generate_recommendations(
        self, metrics: DriftMetrics, severity: DriftSeverity, drift_details: list[DriftDetail]
    ) -> list[str]:
        """
        ...
        if severity == DriftSeverity.CRITICAL:
            ...
            recommendations.append(
                "LOW: Minor architectural changes detected. Changes appear to be within acceptable limits."
            )
        ...
        if metrics.complexity_delta > 100:
            recommendations.append(
                f"Complexity increased by {metrics.complexity_delta}. Consider refactoring to maintain code quality."
            )
        elif metrics.complexity_delta < -100:
            recommendations.append(
                f"Complexity decreased by {abs(metrics.complexity_delta)}. Good progress on code simplification."
            )
        ...
        if metrics.removed_nodes > 10:
            recommendations.append(
                f"{metrics.removed_nodes} code entities removed. Update documentation to reflect removed components."
            )

        # General recommendation
        if metrics.total_changes > 50:
            recommendations.append("Consider updating the baseline to reflect the current architecture state.")
