...
class ComplianceReport:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "characteristics": {name: char.to_dict() for name, char in self.characteristics.items()},
...
class ComplianceVerifier:
    ...
    async def _get_project_nodes(self, driver: Any, project_id: str) -> list[dict[str, Any]]:
    ...
    async def _get_project_relationships(self, driver: Any, project_id: str) -> list[dict[str, Any]]:
    ...
    async def _check_functional_suitability(
        ...
        if violations:
            recommendations.append("Implement proper error handling in all functions with business logic")
            recommendations.append("Complete all empty or stub implementations")
    ...
    async def _check_performance_efficiency(
        ...
        if violations:
            recommendations.append("Refactor high-complexity functions to improve performance")
            recommendations.append("Break down large functions into smaller, focused units")
            recommendations.append("Reduce coupling by minimizing dependencies")
    ...
    async def _check_compatibility(
        ...
        interface_count = sum(1 for n in nodes if n["properties"].get("type") == "interface")
        class_count = sum(1 for n in nodes if n["properties"].get("type") == "class")
        ...
        if class_count > 10 and interface_count == 0:
            ...
            score -= 5
            recommendations.append("Define interfaces to improve interoperability and testability")

        if circular_deps:
            recommendations.append("Resolve circular dependencies to improve module independence")
    ...
    async def _detect_circular_dependencies(self, relationships: list[dict[str, Any]]) -> list[list[str]]:
    ...
    async def _check_usability(
        ...
        if violations:
            recommendations.append("Follow naming conventions for better code readability")
            recommendations.append("Add documentation to all public classes and functions")

        return QualityCharacteristic(
            name="Usability", score=max(0, score), max_score=100, violations=violations, recommendations=recommendations
        )
    ...
    async def _check_reliability(
        ...
        if violations:
            recommendations.append("Implement comprehensive error handling in all critical functions")
            recommendations.append("Add fault tolerance mechanisms for external service calls")
            recommendations.append("Implement recovery procedures for failure scenarios")
    ...
    async def _check_security(
        ...
        auth_functions = sum(
            1 for n in nodes if any(kw in n["properties"].get("name", "").lower() for kw in ["auth", "login", "verify"])
        )
        ...
        if violations:
            recommendations.append("Implement proper error handling in all security-sensitive functions")
            recommendations.append("Add input validation to prevent injection attacks")
            recommendations.append("Implement authentication and authorization mechanisms")

        return QualityCharacteristic(
            name="Security", score=max(0, score), max_score=100, violations=violations, recommendations=recommendations
        )
    ...
    async def _check_maintainability(
                ...
                violations.append(
                    {
                        "entity": name,
                        "type": node_type,
                        "issue": "High complexity reduces maintainability",
                        "severity": "high",
        ...
        if violations:
            ...
            recommendations.append("Break down large entities into smaller, focused modules")
            recommendations.append("Reduce coupling between modules")
            recommendations.append("Add unit tests to improve testability")
    ...
    async def _check_portability(
        ...
        platform_keywords = ["windows", "linux", "darwin", "win32", "posix", "c:\\", "/usr/", "/home/", "d:\\"]
        ...
        for node in nodes:
            ...
            has_platform_code = any(keyword in name or keyword in file_path for keyword in platform_keywords)
        ...
        if violations:
            recommendations.append("Use environment variables or configuration files instead of hardcoded paths")
            recommendations.append("Abstract platform-specific code behind interfaces")
            recommendations.append("Use cross-platform libraries and APIs")
    ...
    def _extract_violations(self, characteristic: QualityCharacteristic, char_name: str) -> list[ComplianceViolation]:
    ...
    def _calculate_overall_score(self, characteristics: dict[str, QualityCharacteristic]) -> int:
    ...
    def _generate_summary(
        self, overall_score: int, compliance_status: ComplianceStatus, characteristics: dict[str, QualityCharacteristic]
    ) -> str:
        ...
        char_scores = []
        for _name, char in characteristics.items():
    ...
    def _generate_recommendations(
        self, characteristics: dict[str, QualityCharacteristic], violations: list[ComplianceViolation]
    ) -> list[str]:
        ...
        if high_violations:
            recommendations.insert(
                1 if critical_violations else 0, f"HIGH PRIORITY: Resolve {len(high_violations)} high-severity issues"
            )
