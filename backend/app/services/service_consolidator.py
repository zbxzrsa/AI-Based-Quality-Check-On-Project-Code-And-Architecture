...
class ServiceConsolidator:
                ...
                for i, service1 in enumerate(services):
                    for service2 in services[i + 1 :]:
                        ...
                        self.dependencies.extend(
                            [
                                ServiceDependency(
                                    source=service1, target=service2, dependency_type="database", critical=True
                                ),
                                ServiceDependency(
                                    source=service2, target=service1, dependency_type="database", critical=True
                                ),
                            ]
                        )
                ...
                for i, service1 in enumerate(services):
                    for service2 in services[i + 1 :]:
                        self.dependencies.append(
                            ServiceDependency(source=service1, target=service2, dependency_type="config")
                        )
        ...
        for dep, services in shared_deps.items():
            if len(services) > 1 and not dep.startswith("@types/"):
                for i, service1 in enumerate(services):
                    for service2 in services[i + 1 :]:
                        self.dependencies.append(
                            ServiceDependency(source=service1, target=service2, dependency_type="shared_library")
                        )
    ...
    def _build_dependency_graph(self) -> DependencyGraph:
        ...
        return DependencyGraph(nodes=nodes, edges=self.dependencies, clusters=clusters)
    ...
    def _dfs_cluster(self, service: str, adjacency: dict[str, set[str]], visited: set[str], cluster: list[str]):
    ...
    def identify_consolidation_candidates(self) -> list[ConsolidationPlan]:
        ...
        if len(existing_ai_services) > 1:
            plan = self._create_consolidation_plan(
                "ai_consolidation", ConsolidationStrategy.MERGE_SIMILAR, existing_ai_services, "ai-service"
            )
            plans.append(plan)
        ...
        if len(existing_pm_services) > 1:
            plan = self._create_consolidation_plan(
                "pm_consolidation", ConsolidationStrategy.MERGE_INTO_CORE, existing_pm_services, "backend-core"
            )
            plans.append(plan)
    ...
    def _create_consolidation_plan(
        self, plan_id: str, strategy: ConsolidationStrategy, source_services: list[str], target_service: str
    ) -> ConsolidationPlan:
        ...
        migration_steps = [
            ...
            "2. Analyze and map all endpoints from source services",
            f"3. Create unified service structure in {target_service}",
            "4. Migrate core functionality while preserving all features",
            "5. Update service registry and routing configuration",
            "6. Update client references and API calls",
            "7. Run comprehensive integration tests",
            "8. Deploy with blue-green strategy",
            "9. Monitor and validate functionality",
            "10. Decommission old services",
        ]
    ...
    def _create_overlap_consolidation_plan(self, overlap: ServiceOverlap) -> ConsolidationPlan:
        ...
        return self._create_consolidation_plan(
            f"overlap_{overlap.service1}_{overlap.service2}", ConsolidationStrategy.MERGE_SIMILAR, [source], target
        )
    ...
    async def merge_services(self, plan: ConsolidationPlan) -> MergeResult:
        ...
        try:
            ...
            updated_references.extend(["api-gateway/src/routes/index.ts", "api-gateway/src/services/serviceProxy.ts"])
