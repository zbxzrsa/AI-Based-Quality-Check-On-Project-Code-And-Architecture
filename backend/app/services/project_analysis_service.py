...
class ProjectAnalysisService:
    ...
    async def _get_pull_requests(self, project_id: str) -> Sequence[PullRequest]:
        ...
        result = await self.db.execute(select(PullRequest).filter(PullRequest.project_id == project_id))
        return result.scalars().all()

    async def _get_review_comments(self, project_id: str) -> Sequence[ReviewComment]:
        """Get all code review comments for a project"""
        prs_result = await self.db.execute(select(PullRequest.id).filter(PullRequest.project_id == project_id))
        ...
        result = await self.db.execute(
            select(ReviewComment).join(CodeReview).filter(CodeReview.pull_request_id.in_(pr_ids))
        )
        return result.scalars().all()
    ...
    def _calculate_metrics(
        self, prs: Sequence[Any], comments: Sequence[Any], violations: Sequence[Any]
    ) -> dict[str, int]:
        ...
        reviewed_prs = [
            ...
            or getattr(pr, "status", "").lower() in ["reviewed", "approved", "merged", "closed"]
        ]
        ...
        risk_scores_raw = [
            getattr(pr, "risk_score", None) for pr in reviewed_prs if getattr(pr, "risk_score", None) is not None
        ]
    ...
    async def _get_recent_reviews(self, project_id: str, limit: int = 5) -> list[dict[str, Any]]:
        ...
        analyzed_prs = []
        for pr in prs:
            if getattr(pr, "analyzed_at", None) or getattr(pr, "status", "").lower() in [
                "reviewed",
                "approved",
                "merged",
                "closed",
        ...
        analyzed_prs.sort(
            key=lambda x: getattr(x, "analyzed_at", None) or getattr(x, "created_at", datetime.min), reverse=True
        )
