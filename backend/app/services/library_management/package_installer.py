...
from app.schemas.library import InstallationResult, InstalledLibrary, LibraryMetadata
...
class CommandExecutor:
    ...
    async def execute_command(
        self, command: list[str], working_dir: str, timeout: float = 300.0
    ) -> tuple[int, str, str]:
        """
        ...
        try:
            ...
            process = await asyncio.create_subprocess_exec(
                *command, cwd=str(full_working_dir), stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
            )
...
class PackageInstaller:
    ...
    async def install(
        self, library: LibraryMetadata, project_context: ProjectContext, version: str | None = None
    ) -> InstallationResult:
        """
        ...
        try:
            logger.info(f"Starting installation of {library.name}@{install_version} in {project_context.value} context")
            ...
            await self._update_dependency_file(
                config["dependency_file"], library.name, install_version, library.registry_type
            )

            # Step 3: Execute install command
            return_code, stdout, stderr = await self.command_executor.execute_command(
                config["install_command"], config["working_dir"]
            )

            if return_code != 0:
                raise CommandExecutionError(
                    f"Package manager install failed with return code {return_code}. stderr: {stderr}"
                )
            ...
            return InstallationResult(success=True, installed_library=installed_library, errors=[])
            ...
            return InstallationResult(success=False, installed_library=None, errors=[str(e)])

    async def _update_dependency_file(
        self, file_path: str, library_name: str, version: str, registry_type: RegistryType
    ) -> None:
        """
    ...
    async def _update_package_json(self, file_path: str, library_name: str, version: str) -> None:
        """
    ...
    async def _update_requirements_txt(self, file_path: str, library_name: str, version: str) -> None:
        """
    ...
    async def _verify_installation(self, library_name: str, project_context: ProjectContext) -> None:
        """
        ...
        try:
            ...
            return_code, stdout, stderr = await self.command_executor.execute_command(
                config["verify_command"], config["working_dir"]
            )

            if return_code != 0:
                raise InstallationVerificationError(
                    f"Verification command failed with return code {return_code}. stderr: {stderr}"
                )

            # Check if library appears in output
            if library_name not in stdout:
                raise InstallationVerificationError(f"Library {library_name} not found in installed packages")
