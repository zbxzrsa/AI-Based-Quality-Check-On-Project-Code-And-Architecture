...
from app.schemas.library import Dependency, InstalledLibrary
...
class LibraryRepository:
    ...
    async def get_libraries_by_project(
        self, project_id: str, context: ProjectContext | None = None
    ...
    async def get_library_by_name(self, project_id: str, name: str, context: ProjectContext) -> InstalledLibrary | None:
        """
        ...
        try:
            query = select(Library).where(
                and_(Library.project_id == project_id, Library.name == name, Library.project_context == context)
            )
            ...
            if not db_library:
                logger.debug(f"Library not found: {name} in {context.value} context for project {project_id}")
                return None
            ...
            logger.debug(f"Found library: {name}@{library.version} (ID: {library.id})")

            return library

        except Exception as e:
            logger.error(f"Error retrieving library {name} for project {project_id}: {e}")
    ...
    async def update_library_version(self, library_id: int, new_version: str) -> None:
        ...
        try:
            ...
            # Update version
            update_query = update(Library).where(Library.id == library_id).values(version=new_version)

            await self.db.execute(update_query)

            logger.info(
                f"Updated library version: {db_library.name} {db_library.version} -> {new_version} (ID: {library_id})"
            )
    ...
    async def save_dependencies(self, library_id: int, dependencies: list[Dependency]) -> None:
        """
        ...
        try:
            ...
            delete_query = delete(LibraryDependency).where(LibraryDependency.library_id == library_id)
            ...
            logger.info(f"Saved {len(dependencies)} dependencies for library {db_library.name} (ID: {library_id})")
    ...
    async def get_library_dependencies(self, library_id: int) -> list[Dependency]:
        """
        ...
        try:
            ...
            # Get dependencies
            deps_query = (
                select(LibraryDependency)
                .where(LibraryDependency.library_id == library_id)
            ...
            for db_dep in db_dependencies:
                dependency = Dependency(
                    name=db_dep.dependency_name, version=db_dep.dependency_version, is_direct=db_dep.is_direct
                )
                dependencies.append(dependency)

            logger.debug(f"Retrieved {len(dependencies)} dependencies for library {db_library.name} (ID: {library_id})")
    ...
    async def delete_library(self, library_id: int) -> bool:
        ...
        try:
            ...
            logger.info(f"Deleted library: {library_name}@{library_version} (ID: {library_id})")
    ...
    async def get_libraries_by_user(self, installed_by: str, project_id: str | None = None) -> list[InstalledLibrary]:
        """
    ...
    async def get_libraries_by_date_range(
        self, project_id: str, start_date: datetime, end_date: datetime, context: ProjectContext | None = None
    ) -> list[InstalledLibrary]:
        """
        ...
        try:
            ...
            logger.error(f"Error retrieving libraries by date range for project {project_id}: {e}")
            raise RuntimeError(f"Failed to retrieve libraries: {str(e)}")
