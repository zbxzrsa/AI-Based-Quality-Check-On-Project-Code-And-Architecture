...
class AsyncCircuitBreaker:
    ...
    async def call(self, func, *args, **kwargs):
        ...
        if self.state == "OPEN":
            if time.time() - self.last_failure_time >= self.recovery_timeout:
                ...
                logger.info("Search circuit breaker entering HALF_OPEN state")
            else:
    ...
    def _on_success(self):
        ...
        if self.state == "HALF_OPEN":
            self.state = "CLOSED"
            logger.info("Search circuit breaker recovered, state: CLOSED")
...
class NPMSearchClient:
    ...
    async def search(self, query: str, limit: int = 20) -> list[LibrarySearchResult]:
        ...
        try:
            response = await self.circuit_breaker.call(self._make_search_request, params)
    ...
    async def _make_search_request(self, params: dict[str, Any]) -> httpx.Response:
        ...
        return await self.http_client.get(self.SEARCH_URL, params=params, timeout=30.0)
...
class PyPISearchClient:
    ...
    async def _try_package_name(self, package_name: str) -> LibrarySearchResult | None:
        """
        ...
        try:
            response = await self.circuit_breaker.call(self._make_request, url)
    ...
    def _parse_package_info(self, data: dict[str, Any]) -> LibrarySearchResult | None:
        """
...
class SearchService:
    ...
    def __init__(self, http_client: httpx.AsyncClient | None = None):
        ...
        self.npm_circuit_breaker = AsyncCircuitBreaker(
            failure_threshold=5, recovery_timeout=60.0, expected_exception=(NetworkSearchError, httpx.RequestError)
        )
    ...
    async def _search_registry(
        self, client: NPMSearchClient | PyPISearchClient, registry: RegistryType, query: str, limit: int
    ) -> list[LibrarySearchResult]:
        """
