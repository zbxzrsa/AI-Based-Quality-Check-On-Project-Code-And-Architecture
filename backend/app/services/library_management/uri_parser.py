...
class URIParser:
    ...
    PATTERNS = {
        RegistryType.NPM: [
            ...
            re.compile(r"^npm:(@?[a-z0-9-]+(?:/[a-z0-9-]+)?)(?:@(.+))?$", re.IGNORECASE),
            # https://npmjs.com/package/name[/v/version]
            # https://www.npmjs.com/package/name[/v/version]
            re.compile(
                r"^https?://(?:www\.)?npmjs\.com/package/(@?[a-z0-9-]+(?:/[a-z0-9-]+)?)(?:/v/(.+))?$", re.IGNORECASE
            ),
        ],
        RegistryType.PYPI: [
            # pypi:package-name[==version]
            # PyPI allows letters, numbers, hyphens, underscores, and periods
            re.compile(r"^pypi:([a-z0-9-_.]+)(?:==(.+))?$", re.IGNORECASE),
            # https://pypi.org/project/name[/version]
            re.compile(r"^https?://pypi\.org/project/([a-z0-9-_.]+)(?:/(.+))?$", re.IGNORECASE),
        ],
        RegistryType.MAVEN: [
            # maven:group:artifact[:version]
            # Maven coordinates use dots for group IDs and hyphens for artifact IDs
            re.compile(r"^maven:([a-z0-9.-]+):([a-z0-9-]+)(?::(.+))?$", re.IGNORECASE),
        ],
    }
        ...
        for registry_type, patterns in self.PATTERNS.items():
            for pattern in patterns:
                ...
                if match:
                    ...
                    return ParsedURI(
                        registry_type=registry_type, package_name=package_name, version=version, raw_uri=uri
                    )
    ...
    def validate_format(self, uri: str) -> tuple[bool, str | None]:
        """
    ...
    def get_registry_type(self, uri: str) -> RegistryType | None:
        """
