...
from app.schemas.library import Dependency, LibraryMetadata
...
class NPMRegistryClient:
    ...
    def extract_metadata(self, package_data: dict, version: str | None = None) -> LibraryMetadata:
        """
        ...
        try:
            ...
            for dep_name, dep_version in deps.items():
                dependencies.append(Dependency(name=dep_name, version=dep_version, is_direct=True))
...
class MetadataFetcher:
    ...
    def __init__(self, http_client: httpx.AsyncClient | None = None):
        ...
        self.npm_circuit_breaker = AsyncCircuitBreaker(
            failure_threshold=5, recovery_timeout=60.0, expected_exception=(NetworkError, httpx.RequestError)
        )
    ...
    async def fetch_metadata(
        self, registry_type: RegistryType, package_name: str, version: str | None = None
    ) -> LibraryMetadata:
        """
        ...
        try:
            ...
            logger.info(
                f"Successfully fetched metadata for {metadata.name}@{metadata.version} from {registry_type.value}"
            )
