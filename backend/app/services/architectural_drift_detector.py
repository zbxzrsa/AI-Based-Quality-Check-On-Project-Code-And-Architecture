...
class ArchitecturalDriftDetector:
    ...
    def _check_layer_dependencies(
        self, module: dict[str, Any], current: dict[str, Any], layer_constraints: dict[str, Any]
    ) -> list[DriftViolation]:
                            ...
                            if target_module:
                                violations.append(
                                    DriftViolation(
                                        ...
                                        suggested_fix="Break circular dependency by introducing interface or moving shared code to common module",
                                    )
                                )
                    ...
                    if source_pattern in source_name and target_pattern in target_name:
                        violations.append(
                            DriftViolation(
                                ...
                                suggested_fix="Remove dependency or refactor to use allowed pattern",
                            )
                        )
                ...
                if len(module_classes) > max_classes_per_module:
                    violations.append(
                        DriftViolation(
                            ...
                            suggested_fix="Split module into smaller modules or move classes to appropriate modules",
                        )
                    )
    ...
    def _generate_recommendations(self, summary: DriftSummary) -> list[str]:
        ...
        if summary.critical_violations:
            recommendations.append(
                "Address critical violations immediately as they pose significant architectural risks"
            )
