...
class AccountLockoutService:
    ...
    async def is_account_locked(self, user_id: int) -> tuple[bool, datetime | None]:
        """
    ...
    async def record_failed_attempt(self, user_id: int) -> tuple[bool, datetime | None]:
        """
        ...
        # Increment attempts
        attempt_count += 1
        await self.redis.setex(attempts_key, timedelta(minutes=self.attempt_window_minutes), attempt_count)
        ...
        if attempt_count >= self.max_attempts:
            ...
            await self.redis.setex(
                lockout_key, timedelta(minutes=self.lockout_duration_minutes), lockout_time.isoformat()
            )
