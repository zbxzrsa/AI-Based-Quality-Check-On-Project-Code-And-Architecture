...
class GDPRComplianceService:
    ...
    async def export_user_data(self, user_id: str) -> bytes:
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zip_file:
            # Add user profile
            zip_file.writestr("profile.json", json.dumps(user_data["profile"], indent=2))

            # Add projects
            zip_file.writestr("projects.json", json.dumps(user_data["projects"], indent=2))

            # Add pull requests
            zip_file.writestr("pull_requests.json", json.dumps(user_data["pull_requests"], indent=2))

            # Add audit logs
            zip_file.writestr("audit_logs.json", json.dumps(user_data["audit_logs"], indent=2))

            # Add data processing consent
            zip_file.writestr("consent_history.json", json.dumps(user_data["consent"], indent=2))
    ...
    async def delete_user_data(self, user_id: str, reason: str = "user_request") -> dict[str, Any]:
        ...
        result = await self.db.execute(delete(ReviewResult).where(ReviewResult.user_id == user_id))
        deletion_report["items_deleted"]["review_results"] = result.rowcount

        # Delete pull requests
        result = await self.db.execute(delete(PullRequest).where(PullRequest.created_by == user_id))
        deletion_report["items_deleted"]["pull_requests"] = result.rowcount

        # Transfer or delete projects
        result = await self.db.execute(delete(Project).where(Project.owner_id == user_id))
    ...
    async def check_data_retention(self) -> list[dict[str, Any]]:
        ...
        result = await self.db.execute(select(AuditLog).where(AuditLog.timestamp < cutoff_date).limit(100))
        ...
        result = await self.db.execute(select(User).where(and_(User.last_login < inactive_cutoff, User.is_active)))
    ...
    async def _collect_user_data(self, user_id: str) -> dict[str, Any]:
        ...
        # Projects
        result = await self.db.execute(select(Project).where(Project.owner_id == user_id))
        projects = [
            {
                "id": str(p.id),
                "name": p.name,
