...
class ContextBuilder:
    ...
    def __init__(self, neo4j_client: Neo4jClient, max_context_tokens: int = 4000, max_depth: int = 2):
        """
    ...
    async def build_context_for_file(
        self, file_path: str, repository: str, include_dependencies: bool = True, include_dependents: bool = False
    ) -> dict[str, Any]:
        """
        ...
        try:
            ...
            result = await self.neo4j_client.execute_read_query(
                file_query, {"file_path": file_path, "repository": repository}
            )
            ...
            if include_dependencies:
                context["dependencies"] = await self._get_file_dependencies(file_path, repository)

            # Get dependents if requested
            if include_dependents:
                context["dependents"] = await self._get_file_dependents(file_path, repository)

            # Get functions in file
            context["functions"] = await self._get_file_functions(file_path, repository)

            # Get classes in file
            context["classes"] = await self._get_file_classes(file_path, repository)

            # Get imports
            context["imports"] = await self._get_file_imports(file_path, repository)
    ...
    async def build_context_for_function(
        ...
        try:
            logger.debug(
                ...
                extra={"function_name": function_name, "file_path": file_path, "repository": repository},
            )
            ...
            result = await self.neo4j_client.execute_read_query(
                function_query, {"function_name": function_name, "file_path": file_path, "repository": repository}
            )
            ...
            if include_callers:
                context["callers"] = await self._get_function_callers(function_name, file_path, repository)

            # Get callees if requested
            if include_callees:
                context["callees"] = await self._get_function_callees(function_name, file_path, repository)

            logger.info(
                f"Context built for function: {function_name}",
                extra={"callers_count": len(context["callers"]), "callees_count": len(context["callees"])},
            )
    ...
    async def build_context_for_class(
        ...
        try:
            logger.debug(
                ...
                extra={"class_name": class_name, "file_path": file_path, "repository": repository},
            )
            ...
            result = await self.neo4j_client.execute_read_query(
                class_query, {"class_name": class_name, "file_path": file_path, "repository": repository}
            )
            ...
            if include_inheritance:
                context["parent_classes"] = await self._get_parent_classes(class_name, file_path, repository)
                context["child_classes"] = await self._get_child_classes(class_name, file_path, repository)

            # Get methods if requested
            if include_methods:
                context["methods"] = await self._get_class_methods(class_name, file_path, repository)
    ...
    async def optimize_context_for_llm(
        self, context: dict[str, Any], priority_fields: list[str] | None = None
    ) -> dict[str, Any]:
        """
        ...
        logger.info(
            f"Context optimized: {len(optimized)} fields retained", extra={"fields_retained": list(optimized.keys())}
        )
    ...
    async def _get_file_dependencies(self, file_path: str, repository: str) -> list[str]:
        ...
        result = await self.neo4j_client.execute_read_query(query, {"file_path": file_path, "repository": repository})

        return [record["dependency"] async for record in result]

    async def _get_file_dependents(self, file_path: str, repository: str) -> list[str]:
    ...
    async def _get_file_functions(self, file_path: str, repository: str) -> list[dict[str, Any]]:
        ...
        return [
            {"name": record["name"], "parameters": record.get("parameters", []), "complexity": record.get("complexity")}
            async for record in result
        ]

    async def _get_file_classes(self, file_path: str, repository: str) -> list[dict[str, Any]]:
        ...
        return [{"name": record["name"], "attributes": record.get("attributes", [])} async for record in result]

    async def _get_file_imports(self, file_path: str, repository: str) -> list[str]:
        """Get imports in file"""
        query = """
    ...
    async def _get_function_callers(self, function_name: str, file_path: str, repository: str) -> list[dict[str, Any]]:
        ...
        result = await self.neo4j_client.execute_read_query(
            query, {"function_name": function_name, "file_path": file_path, "repository": repository}
        )

        return [{"name": record["name"], "file_path": record["file_path"]} async for record in result]

    async def _get_function_callees(self, function_name: str, file_path: str, repository: str) -> list[dict[str, Any]]:
    ...
    async def _get_parent_classes(self, class_name: str, file_path: str, repository: str) -> list[dict[str, Any]]:
        ...
        result = await self.neo4j_client.execute_read_query(
            query, {"class_name": class_name, "file_path": file_path, "repository": repository}
        )

        return [{"name": record["name"], "file_path": record["file_path"]} async for record in result]

    async def _get_child_classes(self, class_name: str, file_path: str, repository: str) -> list[dict[str, Any]]:
    ...
    async def _get_class_methods(self, class_name: str, file_path: str, repository: str) -> list[dict[str, Any]]:
...
def create_context_builder(
    neo4j_client: Neo4jClient, max_context_tokens: int = 4000, max_depth: int = 2
) -> ContextBuilder:
    """
    ...
    """
    return ContextBuilder(neo4j_client=neo4j_client, max_context_tokens=max_context_tokens, max_depth=max_depth)
