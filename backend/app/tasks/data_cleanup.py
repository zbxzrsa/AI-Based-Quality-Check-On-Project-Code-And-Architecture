...
@shared_task(
    name="app.tasks.data_cleanup.cleanup_old_architectural_baselines", bind=True, max_retries=3, default_retry_delay=300
)
            ...
            async for session in get_db():
                try:
                    ...
                    result = await service.cleanup_old_architectural_baselines(
                        dry_run=dry_run, keep_current=keep_current
                    )
                    return result
                finally:
                    await session.close()
...
@shared_task(name="app.tasks.data_cleanup.cleanup_expired_sessions", bind=True, max_retries=3, default_retry_delay=300)
...
@shared_task(
    name="app.tasks.data_cleanup.verify_audit_log_retention", bind=True, max_retries=3, default_retry_delay=300
)
...
@shared_task(name="app.tasks.data_cleanup.get_cleanup_statistics", bind=True, max_retries=3, default_retry_delay=300)
...
@shared_task(
    name="app.tasks.data_cleanup.process_scheduled_account_deletions", bind=True, max_retries=3, default_retry_delay=300
)
            ...
            async for session in get_db():
                try:
                    ...
                    if not pending_deletions:
                        ...
                        return {"status": "success", "processed_count": 0, "dry_run": dry_run}

                    if dry_run:
                        logger.info(f"DRY RUN: Would process {len(pending_deletions)} account deletions")
                        return {"status": "dry_run", "would_process_count": len(pending_deletions), "dry_run": True}
                    ...
                    for deletion in pending_deletions:
                        ...
                        try:
                            ...
                            """)
                            await session.execute(update_deletion, {"user_id": user_id, "completed_at": now})
