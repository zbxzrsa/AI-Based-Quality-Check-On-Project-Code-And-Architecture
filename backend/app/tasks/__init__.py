...
from app.models import Project, PRStatus, PullRequest, ReviewResult
...
@celery_app.task(bind=True, name="app.tasks.analyze_pull_request", max_retries=3, default_retry_delay=60)
    ...
    async def _analyze():
        async with AsyncSessionLocal() as db:
            try:
                ...
                await github_client.get_pull_request(repo_full_name, pr.github_pr_number)
                ...
                for file_data in files:
                    if file_data["status"] in ["added", "modified"]:
                        ...
                        try:
                            ...
                            logger.error(
                                "Error parsing %s: %s", file_data.get("filename", "unknown"), str(e), exc_info=True
                            )
...
@celery_app.task(name="app.tasks.detect_architectural_drift", max_retries=2)
...
@celery_app.task(name="app.tasks.generate_project_documentation", max_retries=1)
