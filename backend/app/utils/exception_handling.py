...
class DetailedException(Exception):
    ...
    def __init__(
        self, message: str, error_code: str, details: dict | None = None, original_error: Exception | None = None
    ):
        """
...
def handle_exception(
    error: Exception, context: dict | None = None, default_message: str = "An error occurred"
) -> DetailedException:
    """
    ...
    error_mapping = {
        ...
        # Network errors
        "ConnectionError": {"code": "CONNECTION_ERROR", "message": "Connection failed", "severity": "high"},
        "TimeoutError": {"code": "TIMEOUT_ERROR", "message": "Operation timed out", "severity": "medium"},
        # Value errors
        "ValueError": {"code": "INVALID_VALUE", "message": "Invalid value provided", "severity": "low"},
        "KeyError": {"code": "KEY_NOT_FOUND", "message": "Required key not found", "severity": "low"},
        ...
        "PermissionError": {"code": "PERMISSION_DENIED", "message": "Permission denied", "severity": "medium"},
    }

    error_info = error_mapping.get(error_type, {"code": "UNKNOWN_ERROR", "message": message, "severity": "medium"})

    return DetailedException(
        message=error_info["message"],
        error_code=error_info["code"],
        details={"original_type": error_type, "context": context or {}, "severity": error_info["severity"]},
        original_error=error,
    )


def with_exception_handling(default_message: str = "An error occurred", reraise: bool = False):
    """
...
@contextmanager
def safe_execute(error_context: dict, default_value: Any = None, reraise: bool = False):
    """
...
def sanitize_string_input(input_string: str, max_length: int = 1000, allowed_chars: str | None = None) -> str:
    """
...
def safe_float_conversion(
    value: Any, default: float = 0.0, min_value: float | None = None, max_value: float | None = None
) -> float:
    """
...
def safe_int_conversion(
    value: Any, default: int = 0, min_value: int | None = None, max_value: int | None = None
) -> int:
    """
...
def retry_on_exception(
    max_retries: int = 3, delay: float = 1.0, backoff: float = 2.0, exception_types: tuple[type, ...] = (Exception,)
):
    """
