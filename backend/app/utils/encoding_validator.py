...
class EncodingValidator:
        ...
        try:
            if not file_path.exists():
                ...
                return EncodingValidationResult(
                    is_valid=False, detected_encoding=None, confidence=0.0, errors=errors, file_path=str(file_path)
                )
            ...
            if not raw_data:
                ...
                return EncodingValidationResult(
                    is_valid=True, detected_encoding="utf-8", confidence=1.0, errors=[], file_path=str(file_path)
                )

            # Detect encoding using chardet
            detection_result = chardet.detect(raw_data)
            detected_encoding = (
                detection_result.get("encoding", "").lower() if detection_result.get("encoding") else None
            )
    ...
    def validate_directory_encoding(
        self, directory_path: Path, pattern: str = "*.py"
    ) -> list[EncodingValidationResult]:
        """
    ...
    def fix_encoding_issues(self, file_path: Path) -> EncodingFixResult:
        ...
        try:
            if not file_path.exists():
                errors.append(f"File does not exist: {file_path}")
                return EncodingFixResult(success=False, original_encoding=None, fixed_content=None, errors=errors)
            ...
            if not raw_data:
                ...
                return EncodingFixResult(success=True, original_encoding="utf-8", fixed_content="", errors=[])
            ...
            if content is None:
                ...
                return EncodingFixResult(
                    success=False, original_encoding=original_encoding, fixed_content=None, errors=errors
                )
        ...
        return EncodingFixResult(
            success=success, original_encoding=original_encoding, fixed_content=fixed_content, errors=errors
        )
