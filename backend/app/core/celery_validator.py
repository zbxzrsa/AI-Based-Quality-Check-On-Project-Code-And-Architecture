...
class CeleryValidator:
    ...
    async def validate(self) -> CeleryValidationResult:
        ...
        if not settings.is_celery_enabled():
            ...
            return CeleryValidationResult(
                is_valid=True, is_enabled=False, warnings=["Celery is disabled - background tasks will not work"]
            )
        ...
        if not settings.CELERY_RESULT_BACKEND:
            ...
            logger.info(
                f"Validating Celery result backend: {ErrorReporter.mask_connection_string(settings.CELERY_RESULT_BACKEND)}"
            )
    ...
    async def _verify_db_backend(self) -> bool:
        ...
        try:
            ...
            engine = create_engine(url, connect_args={"timeout": self.CONNECTION_TIMEOUT}, echo=False)
...
_celery_validator: CeleryValidator | None = None
