...
from app.core.cloudwatch_handler import CloudWatchConfig, create_cloudwatch_handler, get_cloudwatch_status
...
def setup_logging(
    ...
    if enable_json:
        formatter = CustomJsonFormatter("%(timestamp)s %(level)s %(logger)s %(message)s", service_name=service_name)
    else:
    ...
    if enable_cloudwatch:
        cloudwatch_handler = create_cloudwatch_handler(config=cloudwatch_config, log_level=log_level)
    ...
    if enable_cloudwatch:
        ...
        if cw_status["connected"]:
            ...
            logger.warning("CloudWatch Logs integration failed", extra={"error": cw_status["error"]})
...
def log_startup_summary(
    ...
    security_warnings: list | None = None,
) -> None:
    """
...
def set_request_context(
    request_id: str | None = None, user_id: str | None = None, correlation_id: str | None = None
) -> None:
    """
...
async def log_request(request, call_next):
    ...
    set_request_context(request_id=request_id, user_id=user_id, correlation_id=correlation_id)
...
def log_database_status(database_status: dict[str, Any], logger_instance: logging.Logger | None = None) -> None:
    """
    ...
    for db_name, status in database_status.items():
        if isinstance(status, dict):
            ...
            if is_connected:
                logger_instance.info(f"  {db_name}: ✅ Connected ({response_time:.0f}ms)")
            else:
                all_connected = False
                if is_critical:
                ...
                if is_critical:
                    logger_instance.error(f"  {db_name}: ❌ Failed - {masked_error}")
                else:
                    logger_instance.warning(f"  {db_name}: ⚠️  Failed - {masked_error}")
        else:
            # Handle ConnectionStatus objects
            status_obj = status
            if hasattr(status_obj, "is_connected"):
                if status_obj.is_connected:
                    logger_instance.info(f"  {status_obj.service}: ✅ Connected ({status_obj.response_time_ms:.0f}ms)")
                else:
                    all_connected = False
                    ...
                    if status_obj.is_critical:
                        logger_instance.error(f"  {status_obj.service}: ❌ Failed - {masked_error}")
                    else:
                        logger_instance.warning(f"  {status_obj.service}: ⚠️  Failed - {masked_error}")
