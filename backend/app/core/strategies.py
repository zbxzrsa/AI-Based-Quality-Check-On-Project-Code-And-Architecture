...
class MultipartFormStrategy(ContentTypeStrategy):
    ...
    async def validate(self, request: Request) -> dict[str, Any]:
        ...
        try:
            ...
            return {"valid": True, "data": validated_data, "files": file_info}

        except Exception as e:
            logger.warning(f"Multipart form validation failed: {e}")
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid multipart form data")
...
class HTTPSConnectionStrategy(GitHubConnectionStrategy):
    ...
    async def validate_connection(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"valid": True, "connection_type": "https", "clone_url": repo_url}
...
class SSHConnectionStrategy(GitHubConnectionStrategy):
    ...
    async def validate_connection(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"valid": True, "connection_type": "ssh", "ssh_key_id": ssh_key_id, "clone_url": repo_url}
...
class CLIConnectionStrategy(GitHubConnectionStrategy):
    ...
    async def validate_connection(self, config: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"valid": True, "connection_type": "cli", "cli_token": cli_token, "clone_url": repo_url}
...
class DevelopmentValidationStrategy(EnvironmentValidationStrategy):
    ...
    def validate(self, settings: Any) -> list[str]:
        ...
        # 检查是否使用了生产级密钥
        required_secrets = ["AUTH_JWT_SECRET_KEY", "SECRET_KEY", "SESSION_SECRET", "DATABASE_ENCRYPTION_KEY"]
        ...
        for secret_name in required_secrets:
            ...
            if secret_value and secret_value not in insecure_values and len(secret_value) >= 32:
...
class EnvironmentValidator:
    ...
    def validate_environment(self, environment: str, settings: Any) -> dict[str, Any]:
        ...
        if not strategy:
            return {"valid": False, "errors": [f"Unknown environment: {environment}"], "warnings": []}

        issues = strategy.validate(settings)

        # 根据环境类型区分错误和警告
        if environment.lower() == "production":
            return {"valid": len(issues) == 0, "errors": issues, "warnings": []}
        else:
            return {
...
class AuditEventProcessor:
    ...
    def process_event(self, event_category: str, context: dict[str, Any]) -> dict[str, Any]:
        ...
        if not strategy:
            # 默认处理
            return {"event_type": "unknown", "message": f"Unknown event: {event_category}", "severity": "info"}
