...
class InfrastructureMetricsCollector:
    ...
    def __init__(self, namespace: str | None = None, region_name: str | None = None, enabled: bool = True):
        """
        ...
        if BOTO3_AVAILABLE and self.enabled:
            try:
                ...
                logger.info(
                    ...
                    extra={"namespace": self.namespace, "region": self.region_name, "instance_id": self.instance_id},
                )
            except (NoCredentialsError, Exception) as e:
                logger.warning(f"Failed to initialize CloudWatch client: {e}", extra={"error": str(e)})
                self.enabled = False
    ...
    def collect_system_metrics(self) -> dict[str, float]:
        ...
        try:
            ...
            logger.debug("System metrics collected", extra={"metrics_count": len(metrics)})

        except Exception as e:
            logger.error(
                f"Failed to collect system metrics: {e}", extra={"error": str(e), "error_type": type(e).__name__}
            )

        return metrics

    def collect_connection_pool_metrics(
        self, pool_name: str, size: int, checked_out: int, overflow: int, checked_in: int
    ) -> dict[str, float]:
        """
        ...
        logger.debug(
            f"Connection pool metrics collected for {pool_name}", extra={"pool_name": pool_name, "metrics": metrics}
        )

        return metrics

    def collect_cache_metrics(
        self, cache_name: str, hits: int, misses: int, evictions: int, size: int
    ) -> dict[str, float]:
        """
        ...
        logger.debug(
            f"Cache metrics collected for {cache_name}", extra={"cache_name": cache_name, "hit_ratio": hit_ratio}
        )

        return metrics

    def send_metrics_to_cloudwatch(self, metrics: dict[str, float], dimensions: dict[str, str] | None = None) -> bool:
        """
        ...
        try:
            ...
            dimension_list = [{"Name": key, "Value": value} for key, value in default_dimensions.items()]
            ...
            for i in range(0, len(metric_data), batch_size):
                ...
                logger.debug(
                    ...
                    extra={"namespace": self.namespace, "metrics_count": len(batch)},
                )

            logger.info(
                f"Successfully sent {len(metrics)} metrics to CloudWatch",
                extra={"namespace": self.namespace, "metrics_count": len(metrics)},
            )
            ...
            logger.error(
                ...
                extra={"error": str(e), "error_code": error_code, "namespace": self.namespace},
            )
            return False
    ...
    def collect_and_send_all_metrics(self, additional_metrics: dict[str, float] | None = None) -> bool:
        """
...
_metrics_collector: InfrastructureMetricsCollector | None = None
...
def collect_and_send_metrics(additional_metrics: dict[str, float] | None = None) -> bool:
    """
