            ...
            if self.protocol == "tcp":
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.get(
                        f"http://{self.host}:{self.port}", timeout=aiohttp.ClientTimeout(total=self.timeout)
                    ) as response:
...
class ResilientSettings(BaseSettings):
    ...
    def _discover_services(self) -> dict[str, ServiceEndpoint]:
        ...
        endpoints = {
            "postgresql": ServiceEndpoint(host=self.POSTGRES_HOST, port=self.POSTGRES_PORT, required=True),
            "neo4j": ServiceEndpoint(
                host=self.NEO4J_URI.split("://")[1].split(":")[0] if "://" in self.NEO4J_URI else "localhost",
                port=7687,
                required=False,
            ),
            "redis": ServiceEndpoint(host=self.REDIS_HOST, port=self.REDIS_PORT, required=False),
        }
    ...
    def _discover_docker_services(self) -> dict[str, ServiceEndpoint]:
        ...
        try:
            ...
            result = subprocess.run(
                ["docker", "ps", "--format", "{{.Names}}\t{{.Ports}}"], capture_output=True, text=True, timeout=10
            )
                ...
                for line in result.stdout.strip().split("\n"):
                    if "\t" in line:
                        ...
                        if "postgres" in name.lower() and "5432" in ports:
                            docker_endpoints["postgresql"] = ServiceEndpoint(host="localhost", port=5432, required=True)

                        # Parse Redis containers
                        if "redis" in name.lower() and "6379" in ports:
                            docker_endpoints["redis"] = ServiceEndpoint(host="localhost", port=6379, required=False)

                        # Parse Neo4j containers
                        if "neo4j" in name.lower() and "7687" in ports:
                            docker_endpoints["neo4j"] = ServiceEndpoint(host="localhost", port=7687, required=False)
    ...
    async def check_service_availability(self) -> dict[str, bool]:
        ...
        for service_name, endpoint in self._service_endpoints.items():
            task = asyncio.create_task(
                self._check_service_with_timeout(service_name, endpoint), name=f"check_{service_name}"
            )
            tasks.append(task)

        try:
            results = await asyncio.wait_for(
                asyncio.gather(*tasks, return_exceptions=True), timeout=self.SERVICE_DISCOVERY_TIMEOUT
            )
            ...
            availability = dict.fromkeys(self._service_endpoints.keys(), False)
    ...
    def get_degraded_mode_info(self) -> dict[str, Any]:
        ...
        return {
            ...
            "required_services": {service: available.get(service, False) for service in self.REQUIRED_SERVICES},
            "optional_services": {service: available.get(service, False) for service in self.OPTIONAL_SERVICES},
