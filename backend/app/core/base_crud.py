...
class BaseCRUDService(Generic[ModelType, CreateSchemaType, UpdateSchemaType, ResponseSchemaType], ABC):
    ...
    async def check_ownership(self, entity: ModelType, user_id: str, action: str = "access") -> None:
        ...
        if owner_id != user_id:
            logger.warning(
                f"Permission denied: User {user_id} attempted to {action} {self.entity_name} owned by {owner_id}"
            )
            raise HTTPException(
    ...
    async def get_by_id(self, entity_id: str, user_id: str, check_ownership: bool = True) -> ModelType:
        """
        根据ID获取实体
        ...
        result = await self.db.execute(select(self.model).where(self.model.id == entity_uuid))
    ...
    async def create_entity(self, create_data: CreateSchemaType, user_id: str, **kwargs) -> ModelType:
        ...
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create {self.entity_name.lower()}"
            )
    ...
    async def update_entity(self, entity_id: str, update_data: UpdateSchemaType, user_id: str, **kwargs) -> ModelType:
        ...
        try:
            ...
            await self.audit_service.log_action(
                ...
                entity_id=entity_id,
                success=True,
                changes={"original": original_state, "updated": update_dict},
            )
            ...
            await self.db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to update {self.entity_name.lower()}"
            )
    ...
    async def delete_entity(self, entity_id: str, user_id: str, soft_delete: bool = True, **kwargs) -> None:
        ...
        try:
            ...
            await self.db.rollback()
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete {self.entity_name.lower()}"
            )
    ...
    async def preprocess_update_data(
        self, update_dict: dict[str, Any], entity: ModelType, user_id: str, **kwargs
    ) -> dict[str, Any]:
        """预处理更新数据"""
        return update_dict
...
class BaseEndpoint(ABC):
    ...
    def handle_error(self, e: Exception, operation: str) -> HTTPException:
        ...
        if isinstance(e, HTTPException):
            ...
            return HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to {operation}")
