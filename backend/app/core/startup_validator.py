...
class StartupValidator:
    ...
    def validate_security(self) -> list[ValidationError]:
        ...
        if settings.BCRYPT_ROUNDS > 20:
            warning = f"BCRYPT_ROUNDS is {settings.BCRYPT_ROUNDS} (>20). This may significantly impact performance."
            self.result.warnings.append(warning)
            logger.warning(f"Security warning: {warning}")
    ...
    async def validate_migrations(self) -> str | None:
        """
    ...
    def _generate_summary(self) -> None:
        ...
        if self.result.is_valid:
            ...
            # Add database status
            connected_dbs = [status.service for status in self.result.database_status.values() if status.is_connected]
            if connected_dbs:
    ...
    def format_error_report(self) -> str:
        ...
        return ErrorReporter.format_error_report(
            errors=error_messages, warnings=warning_messages, include_remediation=True
        )
    ...
    def log_validation_results(self) -> None:
        ...
        # Log database status
        logger.info("\nDatabase Status:")
        for _service_name, status in self.result.database_status.items():
