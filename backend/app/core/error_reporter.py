...
class ErrorReporter:
    ...
    def log_database_error(
        error_info: DatabaseErrorInfo, logger: logging.Logger | None = None, include_details: bool = True
    ):
        """Static method for backward compatibility"""
        reporter = get_error_reporter()
        if logger:
    ...
    @staticmethod
    def format_structured_error_message(error_info: DatabaseErrorInfo, include_resolution_steps: bool = False) -> str:
    ...
    def report_database_error(
        ...
        # 分类错误
        error_info = self.classifier.classify_error(
            error_message=error_message, component=component, details=details or {}
        )
...
def report_error(
    error: Exception, context: dict[str, Any] | None = None, component: str = "unknown"
) -> DatabaseErrorInfo:
    """便捷函数：报告错误"""
...
def report_database_error(
    error_message: str, component: str = "database", details: dict[str, Any] | None = None
) -> DatabaseErrorInfo:
    """便捷函数：报告数据库错误"""
