...
class SecretsManagerClient:
    ...
    def __init__(self, region_name: str = "us-east-1", cache_ttl_seconds: int = 300, enabled: bool = True):
        """
        ...
        if self.enabled:
            try:
                ...
                logger.warning(
                    f"Failed to initialize AWS Secrets Manager client: {e}. Falling back to environment variables."
                )
                self.enabled = False
                self.client = None
        else:
    ...
    def get_secret(
        self, secret_name: str, key: str | None = None, default: str | None = None, use_cache: bool = True
    ) -> str | None:
        """
        ...
        logger.warning(f"Secret not found in Secrets Manager or environment: {secret_name}")
        return None

    def get_secret_dict(self, secret_name: str, use_cache: bool = True) -> dict[str, Any]:
        """
    ...
    def _extract_key(self, secret_value: str, key: str) -> str | None:
        """
    ...
    def _secret_name_to_env_var(self, secret_name: str, key: str | None = None) -> str:
        """
    ...
    def list_secrets(self, prefix: str | None = None) -> list:
        """
...
# Global instance
_secrets_manager: SecretsManagerClient | None = None
...
def get_secrets_manager() -> SecretsManagerClient:
    ...
    if _secrets_manager is None:
        ...
        _secrets_manager = SecretsManagerClient(region_name=region, cache_ttl_seconds=cache_ttl, enabled=enabled)
