...
class State(ABC):
    ...
    async def handle_action(self, action: str, **kwargs) -> dict[str, Any]:
        ...
        if hasattr(self, method_name):
            ...
            return {"success": False, "error": f"Action '{action}' not supported in state '{self.get_state_name()}'"}
...
class StateMachine(ABC):
    ...
    async def transition_to(self, new_state: str, **kwargs) -> dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "old_state": old_state_name, "new_state": new_state, "entity": self.entity}
    ...
    async def log_state_action(self, action: str, success: bool, **kwargs) -> None:
        ...
        if user_id:
            await UnifiedAuditService.log_action(
                ...
                success=success,
                changes={"state": self.get_state_name(), "action": action, **kwargs},
            )
    ...
    async def handle_accept(self, **kwargs) -> dict[str, Any]:
        ...
        existing_member = await self.context.db.execute(
            select(ProjectMember).where(
                ProjectMember.project_id == self.context.entity.project_id, ProjectMember.user_id == user_id
            )
        )
