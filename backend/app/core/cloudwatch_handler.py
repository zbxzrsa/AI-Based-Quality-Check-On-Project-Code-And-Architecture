...
def create_cloudwatch_handler(
    config: CloudWatchConfig | None = None, log_level: int = logging.INFO
) -> CloudWatchLogHandler | None:
    """
    ...
    try:
        ...
        _ensure_log_group_exists(logs_client, config.log_group, config.retention_days)
        ...
        logger.warning("AWS credentials not found, CloudWatch logging disabled", extra={"error": str(e)})
        return None
        ...
        logger.warning(
            ...
            extra={"error": str(e), "error_code": error_code, "log_group": config.log_group},
        )
        return None

    except Exception as e:
        logger.warning(
            "Unexpected error creating CloudWatch handler", extra={"error": str(e), "error_type": type(e).__name__}
        )
        return None


def _ensure_log_group_exists(logs_client, log_group_name: str, retention_days: int) -> None:
    """
    ...
    try:
        ...
        logger.info(f"Set log retention to {retention_days} days for {log_group_name}")
        ...
        if error_code == "ResourceAlreadyExistsException":
            ...
            try:
                logs_client.put_retention_policy(logGroupName=log_group_name, retentionInDays=retention_days)
                logger.info(f"Updated log retention to {retention_days} days for {log_group_name}")
            except ClientError as retention_error:
                logger.warning(f"Failed to set retention policy: {retention_error}")
        else:
            raise


def add_cloudwatch_handler_to_logger(
    logger_instance: logging.Logger, config: CloudWatchConfig | None = None, log_level: int = logging.INFO
) -> bool:
    """
...
def get_cloudwatch_status(config: CloudWatchConfig | None = None) -> dict:
    """
