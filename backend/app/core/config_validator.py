...
class ConfigValidator:
    ...
    def validate_environment(self, settings: Any):
        ...
        if settings.ENVIRONMENT not in valid_environments:
            self.errors.append(
                f"Invalid ENVIRONMENT: {settings.ENVIRONMENT}. Must be one of: {', '.join(valid_environments)}"
            )
    ...
    def validate_security_settings(self, settings: Any):
        ...
        if not settings.JWT_SECRET or len(settings.JWT_SECRET) < 32:
            self.errors.append(f"JWT_SECRET must be at least 32 characters (current: {len(settings.JWT_SECRET)})")
        ...
        if self.is_production:
            ...
            if any(pattern in settings.JWT_SECRET.lower() for pattern in weak_patterns):
                self.errors.append("JWT_SECRET appears to be a default/development value in production")

        # JWT expiration
        if settings.JWT_EXPIRATION_HOURS > 168:  # 1 week
            self.warnings.append(f"JWT_EXPIRATION_HOURS is very long ({settings.JWT_EXPIRATION_HOURS} hours)")

        # Bcrypt rounds
        if settings.BCRYPT_ROUNDS < 12:
            self.errors.append(f"BCRYPT_ROUNDS must be at least 12 (current: {settings.BCRYPT_ROUNDS})")
        elif settings.BCRYPT_ROUNDS > 20:
            self.warnings.append(f"BCRYPT_ROUNDS is very high ({settings.BCRYPT_ROUNDS}) - may impact performance")
    ...
    def validate_database_config(self, settings: Any):
        ...
        if not (1 <= settings.POSTGRES_PORT <= 65535):
            self.errors.append(f"POSTGRES_PORT must be between 1 and 65535 (current: {settings.POSTGRES_PORT})")
        ...
        if settings.NEO4J_URI:
            if not settings.NEO4J_URI.startswith(("bolt://", "neo4j://", "bolt+s://", "neo4j+s://")):
                self.errors.append("NEO4J_URI has invalid protocol (must start with bolt:// or neo4j://)")
    ...
    def validate_redis_config(self, settings: Any):
        ...
        if not (1 <= settings.REDIS_PORT <= 65535):
            self.errors.append(f"REDIS_PORT must be between 1 and 65535 (current: {settings.REDIS_PORT})")
    ...
    def validate_api_keys(self, settings: Any):
        ...
        if settings.GITHUB_TOKEN and not settings.GITHUB_WEBHOOK_SECRET:
            self.warnings.append(
                "GITHUB_TOKEN is set but GITHUB_WEBHOOK_SECRET is not. Webhook verification will fail."
            )
    ...
    def validate_cors_config(self, settings: Any):
        ...
        if self.is_production:
            localhost_origins = [
                origin for origin in settings.ALLOWED_ORIGINS if "localhost" in origin or "127.0.0.1" in origin
            ]
            if localhost_origins:
                self.warnings.append(f"CORS allows localhost origins in production: {localhost_origins}")

        # Check credentials with wildcard
        if settings.CORS_ALLOW_CREDENTIALS and "*" in settings.ALLOWED_ORIGINS:
            self.errors.append("CORS allows credentials with wildcard origin. This is not allowed by browsers.")
    ...
    def validate_ssl_config(self, settings: Any):
        ...
        if self.is_production and not settings.SSL_ENABLED:
            self.warnings.append(
                "SSL/TLS is not enabled in production. Consider enabling or ensure ALB handles SSL termination."
            )
    ...
    def validate_encryption_config(self, settings: Any):
        ...
        if settings.ENCRYPTION_KEY:
            ...
            try:
                ...
                if len(decoded) != 32:
                    self.errors.append(f"ENCRYPTION_KEY must be 32 bytes when decoded (current: {len(decoded)} bytes)")
            except Exception:
        ...
        if self.is_production and not settings.ENCRYPTION_KEY:
            self.warnings.append("ENCRYPTION_KEY not set in production. Data-at-rest encryption will be disabled.")
    ...
    def validate_logging_config(self, settings: Any):
        ...
        if settings.LOG_LEVEL not in valid_log_levels:
            self.errors.append(
                f"Invalid LOG_LEVEL: {settings.LOG_LEVEL}. Must be one of: {', '.join(valid_log_levels)}"
            )
...
def check_database_connectivity(settings: Any) -> dict[str, bool]:
    ...
    """
    results = {"postgresql": False, "neo4j": False, "redis": False}
