...
class DatabaseErrorClassifier:
    ...
    def classify_error(
        self, error_message: str, component: str = "unknown", details: dict[str, Any] | None = None
    ) -> DatabaseErrorInfo:
        """
        分类数据库错误
    ...
    def get_troubleshooting_tips(self, category: DatabaseErrorCategory) -> list[str]:
        ...
        tips = {
            ...
            DatabaseErrorCategory.POOL_EXHAUSTION: ["增加连接池大小", "检查连接泄漏", "优化查询性能", "实施连接池监控"],
            DatabaseErrorCategory.NETWORK_ERROR: ["检查网络连接", "验证主机名和端口", "检查DNS解析", "确认网络路由"],
        }
