...
class UnifiedAuditService:
    ...
    async def log_auth_failure(
        ...
        await UnifiedAuditService.log_action(
            ...
            user_agent=user_agent,
            success=False,
            changes={"email": email, "failure_reason": failure_reason},
        )
    ...
    @staticmethod
    async def query_logs(db: AsyncSession, filter: AuditFilter) -> list[AuditLog]:
        """
        查询审计日志
    ...
    @staticmethod
    async def get_user_logs(db: AsyncSession, user_id: str | uuid.UUID, limit: int = 100) -> list[AuditLog]:
        """
        获取特定用户的审计日志
    ...
    async def get_failed_attempts(
        db: AsyncSession, ip_address: str | None = None, hours: int = 24, limit: int = 100
    ) -> list[AuditLog]:
        """
        获取失败的认证/授权尝试
            ...
            stmt = select(AuditLog).where(
                and_(
                    ...
                    AuditLog.entity_type.in_(
                        ["authentication_failure", "authorization_failure", "token_refresh_failure"]
                    ),
                )
            )
